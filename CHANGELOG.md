...
The second headline is **Conductor reaches GA**: the enterprise fleet control plane (signed policy-bundle distribution, a per-org/fleet/instance signed-evidence audit sink, and fleet-wide enrollment / remote kill / policy rollback over mTLS + SPIFFE) ships with the operator tooling and documentation to run the full lifecycle — generate the fleet keys, build a signed trust roster, provision PKI (a documented cert-manager bring-your-own-PKI recipe), deploy, enroll followers, publish policy, push a kill, roll back, and verify evidence offline. Followers enforce locally and stay fail-closed; Conductor holds no agent secrets and never scans on their behalf, and every fleet command fails closed without an Enterprise license granting the `fleet` feature.
...
- **Conductor operator workflow — keys, provisioning, and the production lifecycle.** GA is the deploy *and* the day-2 manage. The fleet's five signing-key roles (`policy-bundle-signing`, `remote-kill-signing`, `policy-bundle-rollback`, `audit-batch-signing`, plus the reserved `trust-root-rotation` / `enrollment-token-signing`) are generated with `pipelock signing key generate --purpose <role>`, composed into a signed trust roster with `pipelock signing roster build` (which a follower pins by root fingerprint), and the public halves are wired into `conductor serve` as `inline=` trusted keys. Rollback, remote-kill, and trust-root-rotation are flagged as threshold roles. A documented **bring-your-own-PKI recipe** (cert-manager CA `ClusterIssuer` issuing the auto-renewing Conductor server cert and per-follower client certs whose SPIFFE URI SAN is the fleet identity) provisions production mTLS without hand-rolled OpenSSL. The new [Conductor production runbook](docs/guides/conductor-production-runbook.md) walks the whole lifecycle end to end (generate → provision → deploy → enroll → publish → kill/rollback → rotate → verify offline) with filled-in commands, and is honest about what is not enforced yet (its *Known limitations* section calls out that `conductor.stale_policy` / `strict_deny_all` validates but is not yet enforced by the follower runtime, and that several follower fields remain reserved). The operator *producer* commands for publish / kill / resume / rollback / enrollment-token / fleet-status / audit-query land in companion changes; until then those steps run through the documented HTTP API shapes.
