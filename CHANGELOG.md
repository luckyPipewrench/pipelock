...
- **`pipelock claude-hook` fail-closed coverage extended.** Unsupported hook events now fail-closed instead of falling through to allow. The sibling unknown-tool fall-through is tracked as [PENDING #525] and must replace this placeholder before the v2.5.0 tag. (#524, [PENDING #525])
...
`pipelock-verify-python` is now a parallel, language-portable companion to the new Go, TypeScript, and Rust verifiers; the Python recipe in `docs/guides/` covers v1 chain verification and individual EvidenceReceipt v2 envelopes against the same canonical vectors. EvidenceReceipt v2 chain verification across all 13 payload kinds remains a follow-up across all verifier surfaces.
