...
The second headline is **Conductor moves toward the v2.7 production workflow**: the enterprise fleet control plane (signed policy-bundle distribution, a per-org/fleet/instance signed-evidence audit sink, and fleet-wide enrollment / remote kill / policy rollback over mTLS + SPIFFE) now has the operator documentation and companion command work needed to close the lifecycle — generate the fleet keys, build a signed trust roster, provision PKI, deploy, enroll followers, publish policy, push a kill, roll back, observe followers, and verify evidence offline. Followers enforce locally and stay fail-closed; Conductor holds no agent secrets and never scans on their behalf, and every fleet command fails closed without an Enterprise license granting the `fleet` feature.
