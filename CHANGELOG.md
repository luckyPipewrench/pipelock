...
- **Coverage certificates can be generated for the default proxy.** `pipelock dashboard coverage-cert generate` previously rejected every real session as "mixed actors" because a normal session interleaves the `pipelock` session-control receipts with unattributed (`anonymous`) request receipts. Unattributed traffic is now tolerated only when the requested agent is the default control actor; for a named agent, mixing in unattributed receipts still fails closed so a certificate cannot over-count traffic under a named agent.
- **The durable SIEM forwarder never delivers to cloud-metadata or link-local addresses.** An operator allowlisting a bare IP literal previously bypassed the forwarder's internal-address floor entirely, including the cloud instance-metadata endpoints (`169.254.169.254`, the Azure WireServer, and the IPv6 IMDS address) and link-local ranges. Those ranges are now denied unconditionally even when the destination is an explicitly allowlisted literal; loopback and private (RFC 1918) SIEM destinations remain allowed.
- **Operator dashboard tab icon uses the Pipelock brand mark.** The dashboard favicon now renders the compact Pipelock lock mark instead of the full wordmark logo, which was unreadable at browser-tab size.
...
- **MCP HTTP listener SSE upstream parity.** `pipelock mcp proxy --listen --upstream` now routes `text/event-stream` upstream responses through `SSEReader` so JSON-RPC messages stream to the listener client without waiting for upstream EOF. Closes a regression where SSE-streaming MCP servers (payments, guardrails, etc.) sat silent until the upstream finished or timed out. (#472)
...
- **Frozen config + `ResolveRuntime` clone path.** Runtime policy resolution consolidated into `Config.ResolveRuntime`, eliminating duplicate resolution paths across the CLI and MCP entry points. Loaded config is frozen; runtime policy views are produced by cloning and resolving per-request. (#422)
- **Non-blocking CI debt tracker.** New `hardening-report` job surfaces gocyclo / gocognit / maintidx / dupl metrics per PR without blocking merges, giving contributors visibility into regression trends. (#422)
- **Runtime server lifecycle extraction.** `pipelock run` no longer owns the lifecycle; it delegates to a new `Server` type with `NewServer`, `Start`, `Shutdown`, `Reload`, and `cleanup` methods. Unlocks `internal/cli/runtime/` coverage ceiling. Six spec tests landed. `RegisterKillSwitchSignal` decoupled from Cobra. A polish pass closed two hot-reload gaps: scan API per-request config/scanner/policy resolution, and MCP listener function-pointer-driven per-message snapshotting. (#424)
- **Canonical-hash golden fixtures.** New `canonical_golden_test.go` pins the current `CanonicalPolicyHash` output against a fixed-input config so the mechanical split could not silently invalidate every receipt signed against the current schema. (#425)
- **`internal/config` mechanical split.** The 5,170-LOC `internal/config/config.go` is split into `schema.go`, `defaults.go`, `normalize.go`, `validate.go`, `reloadwarn.go`, `load.go`, and `schema_receiver_methods.go`. Validators now return `[]Warning` instead of writing to stderr. Callers (CLI + verify_install diagnostics) print the returned warnings. No semantic change; canonical-hash golden fixtures prove byte-equivalent policy output. (#431)
