...
- **Structured-fields-safe inbound strip:** `envelope.StripInbound` now parses Signature / Signature-Input as RFC 8941 dictionaries via httpsfv before dropping pipelock-labelled members. The previous `strings.Split(val, ",")` path treated commas inside quoted parameter values as member separators, corrupting surviving members and leaving dictionary residue that could bypass inbound sanitisation. The fail-closed fallback for malformed headers drops any value containing the pipelock namespace (case-insensitive `Contains`) so OWS, tab, or line-fold formatting variants cannot smuggle a malformed pipelock member past the strip.
- **Reverse-proxy signing after Director:** reverse-proxy envelope signing now runs in a `http.RoundTripper` wrapper installed on `httputil.ReverseProxy.Transport`, so `@target-uri` reflects the post-Director upstream URL rather than the inbound relative path.
- **Request body plumbing for signing:** transport inject sites hand the already-scanned body bytes to the signer so Content-Digest is computed without a second drain. When request body scanning is disabled but signing is enabled, the envelope emitter drains `req.Body` itself (bounded by `max_body_bytes`) and installs a fresh `GetBody` closure so stdlib can replay the body on 307/308 redirects.
- **Internal identity strip on forward and intercept paths:** the `X-Pipelock-Agent` header and `?agent=` query parameter are removed from every outbound request before it leaves pipelock, so a caller-supplied identity hint cannot bleed through to the destination when `bind_default_agent_identity` is set.
- **Forwarded-IP family strip:** the full set of caller-supplied origin-attribution headers (`X-Forwarded-For`, `X-Real-IP`, `X-Forwarded-Host`, `X-Forwarded-Proto`, `X-Forwarded-Port`, `Forwarded`, `Via`) is scrubbed on both the forward-proxy CONNECT handler and the TLS-intercept request handler. Pipelock knows the verified client IP and must not pass an attacker-supplied lie through to the backend.
- **MCP subprocess subtree teardown (defense in depth):** the wrapped MCP child starts in its own process group (`Setpgid`); pipelock sets `Pdeathsig = SIGTERM` so the kernel tears the direct child down if pipelock itself dies, enables `PR_SET_CHILD_SUBREAPER` so orphaned descendants reparent to pipelock, captures the pgid at `cmd.Start` into a local value that stays valid across `cmd.Wait`, signals the captured pgid from a context-cancellation watcher ahead of the post-Wait SIGKILL backstop, and sweeps `/proc` for adopted descendants after the pgid kill drains. Pipelock-`SIGKILL` (uncatchable) still orphans cooperative grandchildren to PID 1 — documented as deployment-guidance work tracked for v2.2.1.
- **Content-scanner URL redaction (audit logs and client response):** blocks attributed to content-matching scanners (`dlp`, `body_dlp`, `header_dlp`, `mcp_input_scanning`, `response_scan`, `address_protection`, `seed_phrase`, `cross_request_entropy`) truncate the URL/target to `scheme://host/[redacted]` in structured audit logs AND in the fetch proxy's client-facing 403 response body. Previously a DLP block on a secret-carrying query param echoed the raw secret back both places.
- **Flight recorder startup writability probe + mid-run evidence recovery:** pipelock refuses to start when `flight_recorder.dir` is not writable (rather than silently losing the audit trail), and `ensureFile` stats the configured dir on every entry so a mid-run `rm -rf evidence/` is caught, the dir recreated, the stale fd closed, and the writer dropped so the next `OpenFile` lands in the recreated directory. A stderr notice fires on each recovery cycle.
- **Flight recorder signing-key rotation rejected on reload:** hot-reloading a new `flight_recorder.signing_key_path` would break chain verification (entries signed with two different public keys under one `chain_id`). The reloader preserves the previously loaded key, logs a warning telling the operator a restart is required, and leaves the in-flight chain intact.
- **Config reload coalescing:** fsnotify file-change events and SIGHUP signals are deduplicated so a single config mutation fires exactly one reload even when both arrive in the same 2-second window. Single no-op SIGHUPs still emit an ack so operators can see the signal was received.
- **Sandbox `--best-effort` degraded-mode WARNING:** when user namespaces are unavailable and pipelock falls back to `HTTP(S)_PROXY`-only enforcement, a loud startup WARNING fires alongside the `DEGRADED` posture banner on both `pipelock sandbox --best-effort` and `pipelock mcp proxy --sandbox-best-effort`, so operators see the proxy-bypass risk at launch time rather than discovering it in an incident.
- **Broader network-exfil tool policy:** the default tool policy regex for outbound curl-style uploads now covers `-F` / `--form` / `--data-binary` / `--data-raw` / `--data-urlencode` / `--post-file` / `--body-data` / `--body-file` across all six preset configs.
- **Accurate MCP block-response wording:** tool-poisoning blocks (tools/list containing a Dangerous Capability match) and provenance-verification failures use a dedicated `blockResponseReason` helper so operators see "tool poisoning detected in tools/list" or "tool provenance verification failed" rather than the previous "prompt injection detected" misattribution.
- **`pipelock install <dest>` hidden subcommand:** copies the running binary to a destination path. Used by sidecar init containers built on scratch-based images (no `/bin/sh`) to populate a shared-bin volume so the CLI in the sidecar-mate container matches the sidecar proxy byte-for-byte. Rejects symlinks and non-regular destinations and writes atomically via temp-file-rename so a partial copy is never observable at the final path.
