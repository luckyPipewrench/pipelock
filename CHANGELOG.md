...
- **License / CRL / PKI hardening.** Several changes make the single-issuer trust chain rollback-proof and let you run the online signer behind an intermediate:
...
- **Live playground — a real attack, verified offline by anyone.** A local demonstration engine drives a deterministic toy agent through a real Pipelock proxy and produces an offline-verifiable evidence bundle: an allowed request gets a signed receipt, a blocked exfiltration produces a signed block receipt plus an independent signed collector witness attesting nothing arrived, and (in contained mode) a direct-egress bypass is denied at the kernel and recorded in a signed host-containment witness. One all-or-nothing `verify` command, rooted in a single published orchestrator key, checks the whole run after the proxy has stopped — every on-screen claim carries exactly one evidence class and cannot render as a stronger one. A bundle generator and a gated live-chat backend (invite-code gate, fail-closed rate/size/concurrency limits, live receipt-stream seam) extend it toward an interactive demo, and an optional **model-backed agent** can drive the whole demo with a real OpenAI-compatible model: it runs as a separate subprocess that egresses only through the Pipelock proxy (proxy-only transport guard, fail-closed on any direct dial), enforces a per-turn signed-receipt invariant so no narrated mediated action exists without a receipt to prove it, allowlists only the lab targets and the model API host before DNS, and keeps the synthetic canary out of the model prompt so a jailbroken model still cannot exfiltrate or tunnel through an allowlisted endpoint. Demo/evidence tooling, public-safe by construction (synthetic inputs, reserved `.test` hosts, redact-before-sign); not part of the production firewall path. (#784, #785, #795, #802, #804)
