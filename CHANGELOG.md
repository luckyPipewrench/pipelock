...
- **`pipelock keys status` for a unified signing-key inventory.** Pipelock uses distinct Ed25519 keys for distinct purposes (runtime receipts, contract signing, official rules packages, deployment trust roots, and the Conductor control plane), and the information about where each lives was previously spread across several config sections, deployment-level key files, and the trust roster. `keys status` collapses that into one report: for every recognised key purpose it shows the expected source (config field and/or path), whether the key is present and readable by the calling user, whether it parses as the expected key type, the public-key fingerprint, and a per-purpose note (reserved purpose, threshold key, embedded-vs-env precedence). Text and `--json` output; mirrors `doctor`'s root-run readability caveat. The command never prints private key material, only presence, validity, key type, and the public fingerprint. New [`pipelock keys status` reference](docs/cli/keys.md).
