...
- **`pipelock doctor` now flags inert exemptions and semantic config mismatches.** New `config`-surface checks catch exemptions that parse cleanly but no enabled scanner can honor, the failure mode that trains operators to believe a false positive is fixed when the block silently persists. Doctor now warns when: a `suppress:` entry names a pattern that matches no active DLP or response-scanning pattern (inert, suppresses nothing); a `suppress:` entry names a response-scanning pattern while `response_scanning` is disabled; a `suppress:` entry names a DLP pattern while no suppress-consulting proxy scanner (`request_body_scanning`, `response_scanning`, `response_scanning.sse_streaming`) is enabled, with the correct remediation being `dlp.patterns[].exempt_domains` because URL-query DLP does not consult `suppress:`; or `response_scanning.exempt_domains` / `adaptive_enforcement.exempt_domains` is set while its scanner is disabled. Each finding names the correct knob in its `next` action and appears in both text and `--json` output, counted in the summary and exit code. Checks are conservative: a warning is only emitted when inertness is provable from the loaded config.
