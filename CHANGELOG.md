...
- **`pipelock explain <url>` for remediable block explanations.** A new top-level operator command that runs a URL through the scanner pipeline using the loaded config and explains the verdict so a block is fixable with the narrowest correct change. For a blocked URL it reports the scanner/layer, the matching pattern name (DLP/blocklist), which part of the URL was inspected (`url_query` / `host` / `path` / `scheme`), the destination host, the effective config path, and the **exact remediation knob the blocking scanner actually consults**, plus any broader option and its tradeoff. The mapping is correct per scanner: URL DLP names `dlp.patterns[].exempt_domains` and explicitly marks the top-level `suppress:` list as inert for URL DLP; query entropy names `fetch_proxy.monitoring.query_entropy_exclusions` as a separate gate; blocklist, allowlist, SSRF, rate-limit, length, and the immutable core/scheme/CRLF floors each get their own correct guidance. `explain` performs no network access; it runs the pre-resolution layers and flags when a verdict would additionally depend on runtime DNS resolution. Supports `--config`, `--json`, and exits 0 (allowed) / 2 (config or input error) / 3 (blocked). New [`pipelock explain` reference](docs/cli/explain.md).
