...
2. CI requires eight GitHub Actions contexts: **security-scan**, **test (1.26)**, **test (1.27)**, **test-macos**, **lint**, **build**, **govulncheck**, and **codeql**. The advisory **Pipelock CI Summary** rolls them up for readability but isn't a required context. On an ordinary pull request, **test (1.27)** exists as a required check but its Go 1.27 test matrix is skipped and reports success rather than run again; a PR that itself touches `.github/workflows/**`, `go.mod`, `go.sum`, or the guard that checks them runs the full matrix anyway. The full matrix always runs on both Go 1.26 and 1.27 on push to `main`, and both versions are proven again, pinned, in the release workflow before a tag.
