...
| **Multi-Agent Support** | Per-agent profiles selected by identity. A trusted identity comes from a dedicated per-agent listener or a `source_cidrs` match on the connection's own address (graded `bound`), or from the configured default (graded `config-default`); the `X-Pipelock-Agent` header and `?agent=` query param are self-declared: a name that matches a configured profile does select that profile's policy, but the caller chose the name, so a self-declared identity never counts as a trust boundary and is graded `matched` or `self-declared` in every audit record. |
