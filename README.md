...
| **Multi-Agent Support** | Per-agent profiles selected by operator-bound identity on the main proxy listeners. A dedicated listener or a `source_cidrs` match on the connection's own address selects a profile with grade `bound`; a bound configured default uses grade `config-default`. Unless a bound configured default overrides them, the `X-Pipelock-Agent` header and `?agent=` query param keep the caller-supplied name for attribution but use the `_default` or base policy, even when the name matches a configured profile. The MCP reverse listener keeps its listener-level scanner and policy; its resolved identity is attribution only. |
