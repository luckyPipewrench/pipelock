...
| **Multi-Agent Support** | Per-agent profiles selected by identity. A network-bound identity comes from a dedicated per-agent listener or a `source_cidrs` match on the connection's peer address (graded `bound`), or from the configured default (graded `config-default`). The `X-Pipelock-Agent` header and `?agent=` query param are caller-controlled and graded `matched` or `self-declared`; deployments using them must treat every selectable profile as available to any caller. |
