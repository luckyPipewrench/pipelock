...
| **Multi-Agent Support** | Per-agent profiles selected by operator-bound identity. A dedicated listener or a `source_cidrs` match on the connection's own address selects a profile with grade `bound`; a bound configured default uses grade `config-default`. The `X-Pipelock-Agent` header and `?agent=` query param keep the caller-supplied name for attribution but always use the `_default` or base policy, even when the name matches a configured profile. |
