...
| **Session Profiling** | Per-session behavioral analysis for domain bursts. |
...
| **Multi-Agent Support** | Per-agent profiles selected by operator-configured identity on the main proxy listeners. A dedicated listener or a `source_cidrs` match on the connection's own address selects a profile with grade `bound`. A configured default uses grade `config-default` and selects its matching profile; enabling `bind_default_agent_identity` makes it override caller-supplied names. Otherwise, `X-Pipelock-Agent` takes precedence over the configured default, and `?agent=` is used only when neither is present. Header and query names remain attribution-only and use the `_default` or base policy. The MCP reverse listener keeps its listener-level scanner and policy; its resolved identity is attribution only. |
