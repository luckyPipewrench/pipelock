...
- DLP: 62 patterns + BIP-39 seed phrase detection
...
DLP pattern matching on arbitrary text (MCP arguments, request bodies). 62 patterns with Aho-Corasick pre-filter.
...
- DLP regex matching (62 patterns) with pre-filter: ~8μs. Pre-filter alone: ~497ns with zero allocations on clean text.
