...
| Technique | Example | Status | How |
|-----------|---------|----------|-----|
| Basic injection | "Ignore all previous instructions" | Tested | 33 built-in patterns, case-insensitive |
...
| Session-key partitioning | D: rotate a self-declared agent id per request so each piece lands in its own bucket | Tested | CEE and adaptive/session-profiling state are keyed to the verified client IP, not the attacker-controlled `X-Pipelock-Agent` header / `?agent=` query. Rotating the agent id no longer splits a session. |
...
**Adaptive enforcement:** Entropy-budget hits remain visible but do not add to the per-session adaptive score or acquire a stronger action. With `action: warn`, an opaque payload that matches no concrete detector can continue to pass; use `action: block` when the budget must stop it. Fragment-reassembly DLP hits remain concrete adaptive signals. A session already at `block_all` because of concrete evidence denies entropy-only traffic too.

**Partition resistance (session keying):** HTTP-family CEE, adaptive-enforcement, airlock, and behavioral-baseline state accumulates per logical session, and the session key is anchored to the transport peer IP from the proxy's `RemoteAddr`; forwarded-address headers are ignored. Behind another proxy or load balancer, that address identifies the intermediary, not the original end client, so callers sharing it also share the fallback bucket unless a stronger bound identity separates them. The self-declared agent identity (`X-Pipelock-Agent` header or `?agent=` query) is attacker-controllable, so it narrows the bucket only when it is *not* attacker-variable: an infrastructure-bound identity (a per-agent listener) or a single config-default identity. A `source_cidrs` match narrows which agent policy applies, but it does not produce a name-keyed session: matched identities are keyed by client address like self-declared ones, so ratifying a matched name addresses no live profile either. Header- or query-supplied agent names fold to the transport peer IP, so an attacker cannot rotate the agent identifier per request to split a session. Behavioral-baseline profiles follow that same boundary: name-keyed profiles apply only to bound or config-default identities, while self-declared, matched, and unknown identities use a client key in the `ip4-<hex>` or `ip6-<hex>` form. An embedded non-TCP listener encodes its non-IP peer identifier as the reserved `ip-<hex>` form. Ratifying a self-declared name addresses no live profile. This is fail-closed for partitioning: distinct self-declared callers behind one peer IP share state, so one noisy caller can escalate the other. The same HTTP-family key feeds these stateful controls and is shared across fetch, forward proxy, TLS-intercepted CONNECT, WebSocket, and reverse proxy traffic. MCP stdio and non-listener MCP paths use their transport-owned invocation identity.

On the HTTP reverse listener, authenticated stateful MCP controls use an authenticated principal. The configured listener bearer is one shared credential principal; a verifier-backed resolver can subdivide it for OAuth or mTLS deployments. Pipelock derives an opaque, per-listener keyed state identifier and never keys authenticated principal state from `Mcp-Session-Id`, `_meta.clientInfo`, routing headers, forwarded IP headers, or another client-controlled claim. Bearer rotation revokes the prior state epoch. When `mcp_session_binding.listener_require_state_token` is true, a request without a principal or Pipelock-issued session token is refused before upstream rather than running with that control removed. The omitted default is false: unauthenticated traffic uses the isolated `Mcp-Session-Id` compatibility partition and keeps configured scanning.

Protocol revision `2026-07-28` has no initialization exchange or protocol session. Principal-owned state therefore begins on the first authenticated request and needs no Pipelock extension header. The older `Pipelock-Session-Token` remains an isolated compatibility capability for legacy clients only; current requests never receive it. Denial-of-wallet uses the same authenticated principal when available and otherwise follows the configured subject-trust policy; see [subject trust grades](configuration.md#subject-trust-grades).
...
| **Agent identity spoofing** | Any process that can reach pipelock can claim any agent name via `X-Pipelock-Agent` header or `?agent=` query param. | Use listener binding (dedicated ports per agent) or a `source_cidrs` match on the connection's own source address for network-bound identity; both are graded `bound` and neither authenticates the host or process behind the address. Header/query methods trust the caller. Network isolation adds defense in depth. |
