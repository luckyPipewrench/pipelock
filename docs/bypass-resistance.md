...
From protocol revision `2026-07-28` there is no `Mcp-Session-Id` to rotate: that revision removes protocol-level sessions from the Streamable HTTP transport, so MCP accumulation for a conforming client keys on the client IP directly and the rotation partition above is not reachable. Earlier revisions still send the header and still behave as described, and a mixed-revision fleet is the normal state through the deprecation window, so both paths remain live. Denial-of-wallet quota ownership is a separate question and never keyed on the session; see [subject trust grades](configuration.md#subject-trust-grades).
