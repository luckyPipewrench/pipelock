...
- **Fragment reassembly:** buffers outbound payloads (URLs, request bodies, MCP JSON-RPC payloads, WebSocket frames) per session and re-scans the concatenated content against DLP patterns on every request (synchronous, pre-forward). When the concatenated buffer matches a DLP pattern, the completing request is blocked before it leaves. Buffer size is capped per session to prevent memory exhaustion.
