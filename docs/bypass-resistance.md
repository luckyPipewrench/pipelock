...
| **Agent identity spoofing** | Any process that can reach pipelock can claim any agent name via `X-Pipelock-Agent` header or `?agent=` query param. | Use listener binding (dedicated ports per agent) or a `source_cidrs` match on the connection's own source address for spoof-proof identity; both are graded `bound`. Header/query methods trust the caller. Network isolation adds defense in depth. |
