...
- **Unknown pattern name.** A `suppress:` entry whose `rule` matches no active DLP pattern (`dlp.patterns` plus defaults) and no `response_scanning.patterns` name suppresses nothing. **Fix:** correct the rule name to match a real pattern, or remove the entry.
