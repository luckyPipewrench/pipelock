...
| `entropy` (query entropy) | A high-entropy query key/value crossed the threshold | `fetch_proxy.monitoring.query_entropy_exclusions` (exact or `*.example.com`). **Separate gate from DLP** — exempting a DLP pattern does not lift an entropy block. | Raising `fetch_proxy.monitoring.entropy_threshold` lowers sensitivity for every destination. |
