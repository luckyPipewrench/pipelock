...
- Free-tier summaries redact MCP server names and are signed on the same terms as the paid report. The paid-tier assessment includes full server identity for the operator's own consumption. If that detail should not leave the organization, redact a COPY and share that: `pipelock assess verify` hashes the artifacts the manifest lists, so editing any one of those makes the bundle fail its own verification.
