...
- AC-4 / SI-10: v3.4 reverse-proxy media request bodies are inspected before forwarding, and oversized bodies fail closed rather than forwarding an unverified prefix.
- CM-3 / SI-4: v3.4 removes inert airlock trigger fields and makes scan instrumentation errors distinct from policy findings in CI.
...
| AC-4(4) | Content Check | The ordered scanner pipeline applies destination, DLP, entropy, injection, and SSRF controls where each transport exposes the relevant content. Configured MCP upstreams are an exception to private-address SSRF blocking: local/private servers are allowed, but cloud metadata endpoints remain blocked. Plain CONNECT without TLS interception remains hostname-only. Full-schema tool-poisoning detection applies to mediated MCP tool metadata. | **Strong** |
| AC-6 | Least Privilege | Per-agent profiles constrain each agent to specific listeners, rate limits, and data budgets. Sandbox Landlock restricts filesystem to declared paths only. Seccomp restricts syscalls to an allowlist on linux/amd64; other architectures report the layer unavailable. | **Strong** |
...
| CM-7 | Least Functionality | Strict mode restricts to allowlisted API domains only. On linux/amd64 the seccomp syscall allowlist blocks unnecessary kernel operations; other architectures report the layer unavailable. Sandbox restricts filesystem to declared paths. | **Strong** |
...
| Control | Name | Pipelock Feature | Coverage |
|---------|------|-----------------|----------|
| SC-4 | Information in Shared Resources | Per-agent profiles isolate budgets, rate limits, and session state. Sandbox provides process-level isolation via Landlock and network namespaces, plus seccomp on linux/amd64. | **Strong** |
