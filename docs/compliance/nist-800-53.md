...
**Last updated:** May 2026 (reviewed against v2.4 feature set; v2.4 adds learn-and-lock per-agent behavioral contracts with signed `EvidenceReceipt v2` lifecycle and shadow envelopes for CA-2/CA-7 Continuous Monitoring and AU-2/AU-10 Audit Generation/Non-Repudiation, inbound mediation envelope verification with replay protection for AU-9(2)/(3) Audit Information Protection and IA-2/IA-3 Identification, SPIFFE actor format with `/.well-known/http-message-signatures-directory` per RFC 9421 for IA-2/SC-12 Cryptographic Key Management, the `X-Pipelock-Block-Reason` response header on every HTTP-capable block path (with the same fixed reason vocabulary on JSON-RPC error metadata for MCP-internal blocks) strengthening AU-2 Audit Events and SC-7 Boundary Protection visibility, and Gemini provider redaction with the provider plugin shape extending SI-12 / SC-28 coverage. Builds on the v2.3.0 baseline (class-preserving request redaction for SI-12 / SC-28, generic SSE streaming with per-event body scanning) and the v2.2.0 baseline (mediation envelope for AU-3(1), expanded signed action receipt coverage across all transports for AU-2/AU-10, taint-aware policy escalation for SI-10, posture verify CLI + CI gate for CA-2/CA-7, companion-proxy deployment for SC-7).
