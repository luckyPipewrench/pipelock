...
**Last updated:** May 2026 (reviewed against v2.4 feature set; v2.4 adds learn-and-lock per-agent behavioral contracts strengthening MCP02 Authorization Hijacking & Bypass coverage through operator-ratified behavioral envelopes and signed activation state, inbound mediation envelope verification + replay cache strengthening MCP04 Insufficient Verification of Tool Identity, the `X-Pipelock-Block-Reason` response header on every HTTP-capable MCP block path with the same fixed reason vocabulary on JSON-RPC error metadata for MCP-internal blocks (so the agent sees structured rejection codes regardless of MCP transport — MCP01/MCP02/MCP03 visibility), and Gemini provider redaction extending MCP01 Token Mismanagement & Secret Exposure to a third upstream provider. Builds on the v2.3.0 baseline (class-preserving redaction on MCP `tools/call` `params.arguments` across stdio / HTTP / reverse proxy / WebSocket, generic SSE streaming with per-event body scanning) and the v2.2.0 baseline (mediation envelope, signed action receipts across all MCP transports, taint-aware policy escalation with task boundaries, media policy and SVG active-content hardening, posture verify CLI, companion-proxy deployment via `pipelock init sidecar`).
