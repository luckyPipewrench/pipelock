...
| `issuer_bound_session_cookies` | `true` | Leave a Cookie pair out of header DLP when the same trusted agent session received that exact name and value in a `Set-Cookie` on an intercepted HTTPS response that was delivered, and returns it to the same host and port over HTTPS within the cookie's path and expiry. A value the destination issued tells it nothing new. Every other cookie pair, header, URL and body is scanned as before, as is the value sent to any other host, over cleartext HTTP, or through CONNECT without interception. Takes effect only with `tls_interception.enabled` and header scanning; otherwise inert. Set `false` to scan every cookie. Evidence is memory-only and cleared on reload. See [false-positive tuning](guides/false-positive-tuning.md#browser-session-cookies-on-intercepted-https). |
