...
If top-level `redaction.enabled` is also set, input DLP runs first and an immutable core credential hard-blocks the request before any rewrite: a core credential is never redacted and forwarded on the MCP input path. Only when the request carries no core finding are `tools/call` `params.arguments` rewritten through the same matcher, so a non-core credential in warn mode still forwards the redacted payload instead of the original secret. The behavior is identical across stdio, HTTP/SSE upstream mode, HTTP listener mode, and MCP-over-WebSocket.
