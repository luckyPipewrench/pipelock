...
When require mode is on, the CRL is also subject to a **freshness** check: a CRL whose signed issue time is older than 25 hours (`DefaultCRLMaxAge`) is rejected as stale and fails closed, even if its own expiry has not passed. Publish CRLs on at least a **daily cadence** while require mode is active, independent of the CRL's longer signed validity window — otherwise require-mode consumers will block once the last published CRL ages past 25 hours. Full migration sequence: the [intermediate-signing migration runbook](guides/license-intermediate-migration.md).
