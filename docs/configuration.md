...
**MCP required control:** `response_scanning.enabled: false` is currently ignored in `pipelock mcp proxy` and `pipelock mcp scan` modes because MCP response scanning is required. Pipelock runs with default response scanning and prints a warning naming the overridden field. `pipelock mcp scan` scans stdin responses for prompt injection and generic inbound credential patterns. It intentionally skips agent-owned environment and file-secret matching on visible text because receiving an agent-owned value is not exfiltration. Numeric leaves of a structured response travel on a separate channel that is compared only against known values: a registered canary token (as a plain number or spelled out as decimal character codes) and a configured environment or file secret disguised as decimal character codes. Numbers never enter the prompt-injection or pattern-DLP scan, so ordinary telemetry, pixel data and metric samples cannot be joined into a credential-shaped string, while a protected value delivered entirely as numbers is still found. It does not run tool policy or input scanning. JSON verdicts include `scanned: ["response_injection", "response_dlp"]` to make that scope explicit. This compatibility fallback will become a startup/reload error in a future release; remove the disable to silence the warning.
