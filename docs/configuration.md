...
| `strip_hidden_traps` | bool | `true` | Remove hidden prompt-trap DOM content: a CSS-hidden `div`, `span` or `p` holding only text, an `aria-hidden` element, or an HTML comment whose text carries instruction words such as "ignore" or "disregard". A hidden element that contains markup is interface (applications hide whole views until their scripts reveal them) and is left in place; its text is still covered by response scanning |
