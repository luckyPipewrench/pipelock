...
**What the URL entropy gate measures in protocol-shaped values.** Three shapes that protocols require are measured more precisely, with no configuration needed:
...
- A DNS-over-HTTPS request (RFC 8484), either a GET whose only query parameter is `dns` carrying unpadded base64url or a POST with media type `application/dns-message`, is parsed as a DNS message when it parses strictly: every name, record payload, EDNS option and fixed-width field is checked by DLP and the entropy gate, first piece by piece and then as one joined view, so data split across many short labels is still measured. DNS labels are compared case-insensitively. A message that does not parse strictly, including a trailing byte, a padded or duplicated `dns` value, or a query containing `;`, keeps the whole-value check. The message's pieces go through the same DLP pipeline as the rest of the request, so suppressions and credential-audience rules apply, and in a POST a critical credential match blocks even when request-body DLP is set to warn.
