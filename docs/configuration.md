...
**MCP required control:** `response_scanning.enabled: false` is currently ignored in `pipelock mcp proxy` and `pipelock mcp scan` modes because MCP response scanning is the required injection-control path. Pipelock runs with default response scanning and prints a warning naming the overridden field. `pipelock mcp scan` is a stdin response prompt-injection scanner only; it does not run DLP, secret scanning, tool policy, or input scanning. Its JSON verdicts include `scanned: ["response_injection"]` to make that scope explicit. This compatibility fallback will become a startup/reload error in a future release; remove the disable to silence the warning.
...
Enforcing (`action: block`) runtime launch is descriptor-bound on Linux: Pipelock hashes and executes the same open executable, and passes a directly invoked interpreter the same open script it verified. It fails closed when descriptor execution or `/proc/self/exe` is unavailable. Non-Linux platforms, shebang commands, and `/usr/bin/env` wrappers are rejected under enforcement instead of silently downgrading to a path-based launch. `action: warn` logs an explicit warning before any unpinned compatibility fallback.
