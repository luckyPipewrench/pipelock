...
Protocol revision `2026-07-28` does not use `initialize` or protocol sessions. A conforming client sends its listener bearer on each request and receives persistent protection immediately. No Pipelock-specific response header is required. `Mcp-Session-Id`, `_meta.clientInfo`, forwarded client-IP headers, routing names, and other client-declared values never select or rekey security state.
