...
| `issuer_bound_session_cookies` | `true` | Leave a Cookie pair out of header DLP when the same trusted agent session received that exact name and value in a `Set-Cookie` on an intercepted HTTPS response that was delivered, and returns it to the same host and port over HTTPS within the cookie's path and expiry. Every other cookie pair, header, URL and body is scanned as before. Takes effect only with `tls_interception.enabled` and header scanning. Set `false` to scan every cookie. Keyed evidence persists across enabled reloads and restarts at `$XDG_STATE_HOME/pipelock/proxy/issuer-cookies.json` (default `~/.local/state/pipelock/proxy/issuer-cookies.json`), mode `0600` in a `0750` directory. Disabled prerequisites reset evidence; a missing, invalid, or unreadable file scans all cookies. See [false-positive tuning](guides/false-positive-tuning.md#browser-session-cookies-on-intercepted-https). |
