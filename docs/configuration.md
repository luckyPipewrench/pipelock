...
Run `pipelock doctor --config /etc/pipelock/pipelock.yaml` before restarting after filesystem or permission changes. Its `file_sentry` check performs the same recursive walk without installing inotify watches, so it reports inaccessible subtrees without consuming watch descriptors. It fails when strict startup would fail, warns for best-effort degraded coverage, and reports which user performed the check; run it as the service user for a result that applies to that service.
