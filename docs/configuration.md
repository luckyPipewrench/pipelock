...
| `strip_hidden_traps` | bool | `true` | Remove hidden prompt-trap DOM content: a CSS-hidden `div`, `span` or `p` whose text, read with tags removed, carries instruction words such as "ignore" or "disregard", an `aria-hidden="true"` element whose own text carries one of those words, or an HTML comment that does. A tag that only appears inside a comment, an attribute value, or script or style text is not treated as an element. A hidden element that contains interface markup (a form, input, button, link, image, list, table or similar) is an application view waiting to be shown and is left in place; wrapper and inline tags such as `div`, `p`, `b` and `em` do not make an element interface. Kept text is still covered by response scanning |
