...
`pipelock contain install` also installs a root-managed credential guard (`pipelock-cred-guard.path` / `.service`). The path unit wakes on parent-directory changes so atomic temp-file plus rename rewrites cannot slip past it, and also watches the exact credential-shaped files under the operator's home directory, `.claude`, `.claude-cc2`, and `.codex` roots, plus `*.token` globs in those roots. The service then filters the rescan to credential-shaped files only and re-applies the same credential lock if a later tool recreates, replaces, or widens them. The home-directory pass is depth-limited to top-level credential files such as `~/.claude.json`.
