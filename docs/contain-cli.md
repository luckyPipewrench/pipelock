...
The signing key (`flight_recorder.signing_key_path`) is resolved once, before launch, from the same config the posture capsule uses, and that exact key is reused for the statement after launch, an operator rotating the signing key while the agent runs cannot change which key ends up signing this session's statement. If the key cannot be loaded before launch, `contain run` says so up front (before starting the agent) and never attempts the statement afterward.
