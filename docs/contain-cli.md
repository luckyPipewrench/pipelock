...
Newly installed `pipelock.service` units support `sudo systemctl reload pipelock`, which sends SIGHUP and waits for the daemon to finish evaluating the config. Exit status 0 means evaluation finished, not necessarily that the candidate policy was applied. Check `sudo systemctl status pipelock` for the `Status` line and `sudo journalctl -u pipelock` for the verdict. A rejected trust expansion leaves the active policy unchanged and requires `sudo systemctl restart pipelock` to take effect.
