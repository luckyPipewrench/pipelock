...
| **Flight recorder** | Configured evidence storage: per-writer hash-chained JSONL logs with chain continuity and signed checkpoints. It records blocks; allow receipts require the configured receipt mode, and clean stream frames may be summarized rather than emitted one by one. Current releases do not reject multiple writer processes sharing one recorder directory; use `pipelock evidence doctor DIR` to detect structural fork damage. | [Flight Recorder](../guides/flight-recorder.md) |
