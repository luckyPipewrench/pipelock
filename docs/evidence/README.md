...
| **Flight recorder** | Configured evidence storage: per-writer hash-chained JSONL logs with chain continuity and signed checkpoints. It records blocks; allow receipts require the configured receipt mode, and clean stream frames may be summarized rather than emitted one by one. Each process run records its own chain, which the shipped verifiers accept unchanged. Continuity between runs is an optional signed link file beside the chains, not inside them: deleting one makes its successor look unlinked and is not detected, and a structurally clean directory does not prove continuity. Tamper-resistant completeness across runs needs a signed head commitment that does not exist yet. `pipelock evidence doctor DIR` reports structural damage and link findings, and lists unlinked runs. | [One chain per process run](../guides/flight-recorder.md#one-chain-per-process-run) |
...
| **Transcript root** | The hash of the final receipt in a verified writer chain, serving as a tamper-evident summary of the records in that chain. Computing one requires a pinned key. | [Computing a transcript root](../guides/receipt-verification.md#computing-a-transcript-root) |
