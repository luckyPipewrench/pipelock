...
| GET to an AWS S3 presigned URL (issuer's bucket) | core_dlp / dlp | AWS Access ID | None required — handled automatically. The scanner detects a structurally valid SigV4 query set (`X-Amz-Algorithm=AWS4-HMAC-SHA256`, `X-Amz-Credential=<KeyID>/<date>/<region>/<service>/aws4_request`, `X-Amz-Date`, `X-Amz-Signature`) and exempts only the access-key component inside the credential. The same access-key elsewhere in the URL — path, hostname, other query params — still blocks. SigV4 carve-outs are adaptive-neutral: they neither poison the threat score nor earn clean-decay. An `X-Amz-Expires` above 24h attaches an info-tier `SigV4 Long Expiry` warn finding for audit visibility but does not block. |
