...
- Cookies up to 4096 bytes are remembered, per RFC 6265. Evidence has per-session and global limits. The proxy saves the HMAC key and keyed digests in `$XDG_STATE_HOME/pipelock/proxy/issuer-cookies.json` (default `~/.local/state/pipelock/proxy/issuer-cookies.json`). The file is `0600` and its directory is `0750`. Writes are limited to once every three seconds, with a final write on graceful shutdown. A crash can lose the latest few seconds.
