...
- On a later intercepted HTTPS request from the same agent session, Pipelock splits each `Cookie` header into name=value pairs. A pair is left out of the header DLP scan only if that session received exactly that name and value on the same port, the request host is one the cookie's scope covers (described in the next item), the request path is within the cookie's path, and the cookie has not expired. The port requirement is Pipelock's own; browsers do not scope cookies by port. Every other pair in the same header is still scanned, and the forwarded request is not changed.
