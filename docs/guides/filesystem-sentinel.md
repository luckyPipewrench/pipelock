...
When `scan_content` is true (the default), file sentry reads each modified file and runs pipelock's DLP scanner on the content. The same 67 credential patterns used for network traffic apply to file content.
