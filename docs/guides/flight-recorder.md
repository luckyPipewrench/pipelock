...
`transcript_root` receipt commitment. Only one trailing checkpoint may follow that root; it is hash-chain-verified but not covered by the seal, and any other entry after the root makes the result INCOMPLETE. Signed checkpoints (the `sign_checkpoints` default) are verified against the key that was active where each one sits, the signer of the receipt segment it belongs to, including a successor key authorized by a rotation endorsement; they are what authenticate the non-receipt entries before them, and a retired key cannot anchor entries written after its rotation. A checkpoint whose signature verifies against no trusted key, whose span does not match its position, or that is unsigned in a session whose other checkpoints are signed fails verification. The seal must be covered by a signed trailing checkpoint; a recorder where none follows the seal (absent, unsigned, or removed) is refused unless `--allow-unanchored-seal` is passed, since a rewrite could have removed it while an older checkpoint still verifies; with the flag, the output names the entries that are not authenticated. If the writer chain rotated its signing key, pass each public
