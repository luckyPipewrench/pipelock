...
`transcript_root` receipt commitment. Only the trailing checkpoint may follow that root; it is hash-chain-verified but not covered by the seal, and any other entry after the root makes the result INCOMPLETE. Signed checkpoints (the `sign_checkpoints` default) are verified against the pinned keys and are what authenticate the non-receipt entries before them; a checkpoint whose signature verifies against no pinned key, or whose span does not match its position, fails verification. If the writer chain rotated its signing key, pass each public
