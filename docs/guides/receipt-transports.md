...
On the MCP HTTP listener, agent-to-agent (A2A) decision points — the A2A-Extensions header block and A2A method body blocks — now emit a policy-hash-bearing block receipt in the `mcp_a2a_scanning` layer, closing a parity gap where a blocked A2A action left no evidence while the forward proxy already recorded its A2A blocks. By default a synthetic action ID is minted only when an A2A request is actually blocked, so clean/allowed A2A and ordinary `tools/call` traffic emit nothing extra. Under `flight_recorder.require_receipts: true`, a clean A2A method allow additionally mints an action ID and emits an `allow` receipt (with the A2A method name, e.g. `SendMessage`, as `target`), so the provable-allow guarantee covers A2A allows the same way it already covers clean `tools/call` allows. If that required receipt cannot be emitted, the request is blocked fail-closed instead of forwarded.
