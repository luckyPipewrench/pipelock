...
- **Per-agent identification:** agents identify via listener binding or a `source_cidrs` match (both network-bound and graded `bound`: a request cannot change them, though they do not authenticate the source host or process), the bound configured default (graded `config-default`), or the `X-Pipelock-Agent` header and `?agent=` query param (self-declared, never a trust boundary). On the main proxy listeners, bound identities can select config profiles with independent mode, allowlist, DLP, rate limits, and budget. The MCP reverse listener retains its listener-level scanner and policy; its resolved identity is attribution only. Header and query names remain visible for attribution but use the fallback policy: the bound configured default when enabled, otherwise `_default` or the base policy. All log entries include the agent name.
...
**Coverage: Partial.** Ed25519 signing provides agent identity verification for file-level operations. Per-agent profiles with listener binding or `source_cidrs` matching provide network-bound identity for proxy traffic that a request cannot change, given listener access control and source-address integrity. `X-Pipelock-Agent` header and `?agent=` query param provide convenience identification (trust the caller). No certificate-based agent authentication yet.
