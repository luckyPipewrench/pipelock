...
- **Multi-agent identification:** each agent gets a named profile with independent mode, allowlist, DLP, rate limits, and budget. Identity resolves by grade: a per-agent listener binding or a `source_cidrs` match on the connection's own source address is infrastructure-bound and spoof-proof, so it can drive per-agent policy; the `X-Pipelock-Agent` header and `?agent=` query param are self-declared and used for audit and non-authorization matching, not as a trust boundary. All audit log entries include the agent name and its identity grade, enabling per-agent monitoring.
