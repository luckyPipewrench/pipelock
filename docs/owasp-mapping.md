...
- **Multi-agent identification:** each agent gets a named profile with independent mode, allowlist, DLP, rate limits, and budget. Identity resolves by grade: a per-agent listener binding or a `source_cidrs` match on the connection's own source address is infrastructure-bound and cannot be changed by the request, so it can drive per-agent policy on the main proxy listeners (the MCP reverse listener uses a bound identity for audit and per-session state but keeps the listener's own scanner and policy rather than a matched profile's overrides); the `X-Pipelock-Agent` header and `?agent=` query param are self-declared: a matching name selects that profile's policy, but because the caller chose the name it is graded `matched` rather than `bound` and is never a trust boundary, so a deployment that needs network-bound separation binds each agent by listener or source CIDR. All audit log entries include the agent name and its identity grade, enabling per-agent monitoring.
