...
- **Multi-agent identification:** each operator-bound agent can use a named profile with independent mode, allowlist, DLP, rate limits, and budget. A per-agent listener binding or a `source_cidrs` match on the connection's own source address is infrastructure-bound and can drive per-agent policy on the main proxy listeners; a bound configured default can do the same. The MCP reverse listener uses bound identity for audit and per-session state but keeps the listener's own scanner and policy. The `X-Pipelock-Agent` header and `?agent=` query param retain a caller-supplied name for attribution with grade `matched` or `self-declared`, but use the fallback policy: the bound configured default when enabled, otherwise `_default` or the base policy. Audit records include the agent name and identity grade.
