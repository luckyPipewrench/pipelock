...
| DLP pattern match (62 patterns, pre-filtered) | ~5.4 μs | ~184,000/sec |
...
The binary is ~22MB static (release build with symbol stripping). Memory usage is dominated by the DLP regex compilation (~40MB RSS at idle with default patterns) and scales linearly with concurrent connections, not pattern count.
