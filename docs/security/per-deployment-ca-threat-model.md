...
Mitigation: treat CA regeneration as a mandatory post-restore step. For hosts installed with `pipelock contain install`, first run `pipelock contain ca-refresh --dry-run --regenerate-on-snapshot-restore` to preview the affected trust paths, then run `pipelock contain ca-refresh --regenerate-on-snapshot-restore` during the restore workflow. The command requires contain-managed TLS material, backs up the previous `ca.pem` and `ca-key.pem` beside the originals with a `.prerotate.<timestamp>` suffix, force-generates a fresh contain-managed CA as `pipelock-proxy`, exports the new certificate, and rebuilds `/etc/pipelock/combined-ca.pem`. To prevent accidental repeated rotations, a second snapshot regeneration within ten minutes is refused unless the operator passes `--force`; the guard uses `/etc/pipelock/contain/snapshot-ca-refresh.timestamp`, not CA file modification times.

After regeneration, restart agent processes and MCP wrappers that already loaded the old bundle, and update every trust path that points at copied or cached CA material, including `SSL_CERT_FILE`, `REQUESTS_CA_BUNDLE`, `NODE_EXTRA_CA_CERTS`, `CURL_CA_BUNDLE`, and host trust stores. If the old Pipelock root was installed into a host trust store, remove that trust-store entry and install the new exported root. The kill switch can stop new proxy traffic during the maintenance window, but it does not revoke an old CA root from client trust stores or unload CA bundles already held by running processes.
