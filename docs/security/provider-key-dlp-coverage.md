...
Each row has a compiled credential-audience host set. Pipelock allows the matching credential only when the proxy's verified destination host is in that set and the scheme is encrypted (`https` or `wss`). The decision applies to URL, request-body, request-header, and outbound WebSocket-frame DLP, and records `dlp_credential_audience_allow`. A match for the same credential at another host remains blocked. Google OAuth tokens have an additional restriction: only a Bearer `Authorization` header can earn the allow. GitHub token classes earn the allow only on an `Authorization` header with the Bearer or token scheme at `api.github.com` or `uploads.github.com`. `GitLab PAT` earns it only on `PRIVATE-TOKEN` or `Authorization: Bearer` at `gitlab.com`, and `GitLab CI Job Token` only on `JOB-TOKEN`. A separate git-over-HTTPS rule lets `GitHub Token`, `GitHub Fine-Grained PAT`, and `GitLab PAT` travel as `Authorization: Basic` over `https` to the git host (`github.com`, `gitlab.com`, or a declared enterprise host), and only on a git transport path: `<repo>/info/refs?service=git-upload-pack` or `?service=git-receive-pack`, `<repo>/git-upload-pack`, `<repo>/git-receive-pack`, or a Git LFS path under `<repo>/info/lfs/`, as defined by gitprotocol-http(5) and the Git LFS batch and locking API documents. The path is checked as forwarded; a percent-encoded path or one containing `..`, `.`, or `//` segments does not qualify. Basic on any other path, Bearer or token at `github.com`, and Basic at `api.github.com` all block. GitLab Basic is restricted to the same git paths; GitLab Bearer and `PRIVATE-TOKEN` stay valid on any path for the REST API. Other GitLab token classes have no audience. URL, body, other-header, and WebSocket-frame matches for those classes remain blocked. `dlp.github_enterprise_hosts` and `dlp.gitlab_hosts` add exact operator-named hosts for enterprise and self-managed instances. A declared host is trusted with the credential: if it is attacker-controlled, it receives the token by design. AWS secret keys and private keys have no audience. These entries do not trust a whole provider host or use `exempt_domains` or `suppress`. The audience is available without a paid license.
...
| GitHub Token | `gh[pour]_` or `ghs_` | `api.github.com`, `uploads.github.com` (`Authorization: Bearer` or `Authorization: token` only); `github.com` git transport paths (`Authorization: Basic` only) | [GitHub REST authentication](https://docs.github.com/en/rest/authentication/authenticating-to-the-rest-api), [release assets](https://docs.github.com/en/rest/releases/assets) |
| GitHub Fine-Grained PAT | `github_pat_` | `api.github.com`, `uploads.github.com` (`Authorization: Bearer` or `Authorization: token` only); `github.com` git transport paths (`Authorization: Basic` only) | [GitHub REST authentication](https://docs.github.com/en/rest/authentication/authenticating-to-the-rest-api) |
| GitLab PAT | `glpat-` | `gitlab.com` (`PRIVATE-TOKEN` or `Authorization: Bearer` on any path; `Authorization: Basic` only on git transport paths) | [GitLab REST authentication](https://docs.gitlab.com/api/rest/authentication/) |
...
The compiled audience entries do not use suppressions. Immutable core DLP names cannot be suppressed. A core DLP name can still carry a compiled audience: `Slack Token` is allowed only at Slack's exact encrypted API authorities (`slack.com` and `mcp.slack.com`). `GitHub Token` and `GitHub Fine-Grained PAT` are allowed only on an encrypted `Authorization` header at `api.github.com` and `uploads.github.com`, or as `Authorization: Basic` on a git transport path at `github.com`. `GitLab PAT` is allowed only on an encrypted `PRIVATE-TOKEN` or `Authorization: Bearer` header at `gitlab.com`, or as `Authorization: Basic` on a git transport path there. A declared enterprise host also joins the git rule, so the confused-deputy residual for declared hosts covers git traffic too. No operator YAML can suppress those names, set them to warn, or add `exempt_domains`. `dlp.github_enterprise_hosts` and `dlp.gitlab_hosts` are the only way to name an enterprise or self-managed host, and a declared host the operator does not control receives the credential by design. `Google OAuth Token` is a built-in pattern outside the core floor; its compiled audience allows only a Bearer `Authorization` header at encrypted `*.googleapis.com` destinations.
