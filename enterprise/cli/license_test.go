...
// TestRefuseKeyPathAliases covers the guard that stops an issuance output path
// from naming the signing key. The export writer renames over its target and the
// ledger writer appends to it, so either would replace the private key with
// issuance output and still report success.
func TestRefuseKeyPathAliases(t *testing.T) {
	dir := t.TempDir()
	...
	for _, tc := range []struct {
		...
		{name: "export names the key", outputs: map[string]string{"--export": keyPath}, wantErr: "--export must not name the signing key"},
		{name: "ledger names the key", outputs: map[string]string{"--ledger": keyPath}, wantErr: "--ledger must not name the signing key"},
		{name: "export symlinks to the key", outputs: map[string]string{"--export": linkToKey}, wantErr: "--export must not name the signing key"},
		{name: "ledger hard links to the key", outputs: map[string]string{"--ledger": hardLinkToKey}, wantErr: "--ledger must not name the signing key"},
		{name: "relative path names the key", outputs: map[string]string{"--export": filepath.Join(dir, "..", filepath.Base(dir), "license.key")}, wantErr: "--export must not name the signing key"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			err := refuseKeyPathAliases(keyPath, tc.outputs)
			if err == nil {
				t.Fatal("refuseKeyPathAliases accepted an output path naming the signing key")
			}
	...
	t.Run("distinct paths are allowed", func(t *testing.T) {
		if err := refuseKeyPathAliases(keyPath, map[string]string{
			...
			t.Fatalf("refuseKeyPathAliases rejected distinct output paths: %v", err)
		}
	})

	t.Run("empty paths are allowed", func(t *testing.T) {
		if err := refuseKeyPathAliases(keyPath, map[string]string{"--export": "", "--ledger": ""}); err != nil {
			t.Fatalf("refuseKeyPathAliases rejected empty output paths: %v", err)
		}
		if err := refuseKeyPathAliases("", map[string]string{"--export": keyPath}); err != nil {
			t.Fatalf("refuseKeyPathAliases rejected an empty key path: %v", err)
		}
	})
}
