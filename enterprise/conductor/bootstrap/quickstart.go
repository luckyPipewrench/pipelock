...
func writeQuickstart(out io.Writer, r *Result) {
	...
	w("    --trusted-control-key id=%s,purpose=remote-kill-signing,file=%s \\\n", remoteKillSecondKeyID, r.Layout.RemoteKillSecondPubPath)
	...
	w("\n4) Exercise remote kill and resume. Both generated remote-kill signers are\n")
	w("   required for either catastrophic action:\n")
	w("  pipelock conductor kill \\\n")
	w("    --conductor-url %s \\\n", shellQuote(r.ConductorURL))
	w("    --org %s --fleet %s --instance '*' \\\n", shellQuote(r.Identity.OrgID), shellQuote(r.Identity.FleetID))
	w("    --signing-key %s \\\n", shellQuote(r.Layout.RemoteKillKeyPath))
	w("    --signing-key %s \\\n", shellQuote(r.Layout.RemoteKillSecondKeyPath))
	w("    --reason 'bootstrap emergency exercise' --counter 1 --ttl 15m \\\n")
	w("    --admin-token-file %s \\\n", shellQuote(r.Layout.AdminTokenPath))
	w("    --tls-cert %s --tls-key %s \\\n", shellQuote(r.Layout.FollowerClientCertPath), shellQuote(r.Layout.FollowerClientKeyPath))
	w("    --server-ca %s\n", shellQuote(r.Layout.CACertPath))
	...
	w("    --signing-key %s \\\n", shellQuote(r.Layout.RemoteKillKeyPath))
	w("    --signing-key %s \\\n", shellQuote(r.Layout.RemoteKillSecondKeyPath))
	w("    --reason 'bootstrap emergency exercise complete' --counter 2 --ttl 15m \\\n")
	w("    --admin-token-file %s \\\n", shellQuote(r.Layout.AdminTokenPath))
	w("    --tls-cert %s --tls-key %s \\\n", shellQuote(r.Layout.FollowerClientCertPath), shellQuote(r.Layout.FollowerClientKeyPath))
	w("    --server-ca %s\n", shellQuote(r.Layout.CACertPath))
	...
	w("enrollment, threshold-signed remote kill/resume, and a follower-signed audit\n")
	w("batch a Conductor accepted and that verifies offline. Mediation completeness — the agent reaching the network\n")
