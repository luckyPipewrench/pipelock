...
func runAssessFinalize(runDir string, opts assessFinalizeOpts) error {
	...
	if err != nil {
		return ExitCodeError(2, fmt.Errorf("reading manifest: %w", err))
	}

	var manifest AssessManifest
	if err := json.Unmarshal(manifestData, &manifest); err != nil {
		return ExitCodeError(2, fmt.Errorf("parsing manifest: %w", err))
	}

	if manifest.Status == assessStatusFinalized {
		return ExitCodeError(2, fmt.Errorf("already finalized"))
	}
	if manifest.Status != assessStatusCompleted {
		return ExitCodeError(2, fmt.Errorf("status is %q, expected %q", manifest.Status, assessStatusCompleted))
	}
	if len(manifest.SkippedPrimitives) > 0 && !opts.AllowPartial {
		return ExitCodeError(2, fmt.Errorf("assessment has skipped primitives %v; use --allow-partial to finalize", manifest.SkippedPrimitives))
	}
	...
	if err != nil {
		return ExitCodeError(2, fmt.Errorf("reading evidence: %w", err))
	}
	...
	if opts.HasAssess && !opts.Unsigned {
		agentName, err := resolveAgentName(opts.Agent)
		if err != nil {
			return ExitCodeError(1, fmt.Errorf("resolving agent for signing: %w", err))
		}

		dir, err := resolveKeystoreDir(opts.KeystoreDir)
		if err != nil {
			return ExitCodeError(1, fmt.Errorf("resolving keystore: %w", err))
		}
		ks := signing.NewKeystore(dir)

		privKey, err := ks.LoadPrivateKey(agentName)
		if err != nil {
			return ExitCodeError(1, fmt.Errorf("loading key for agent %q: %w", agentName, err))
		}
		...
		if err != nil {
			return ExitCodeError(1, fmt.Errorf("signing manifest: %w", err))
		}
		sigPath := manifestPath + signing.SigExtension
		if err := signing.SaveSignature(sig, sigPath); err != nil {
			return ExitCodeError(1, fmt.Errorf("saving signature: %w", err))
		}
	} else {
...
`, manifest.RunID, now.Format(time.RFC3339), runDir, agentHint, agentHint)
	...
	if err := os.WriteFile(filepath.Join(cleanDir, "verify.txt"), []byte(verifyText), 0o600); err != nil {
		return ExitCodeError(2, fmt.Errorf("writing verify.txt: %w", err))
	}
	...
	if opts.Archive {
		...
		if err := createTarGz(archivePath, cleanDir); err != nil {
			return ExitCodeError(2, fmt.Errorf("creating archive: %w", err))
		}
	}
...
func reconstructSimulateResult(scenarios []ScenarioResult) *SimulateResult {
	if len(scenarios) == 0 {
		return nil
	}
