...
// assessCmd is the parent command grouping all assess subcommands.
...
func runAssessInit(configFile, outputDir string) (string, error) {
	...
	if configFile != "" {
		if _, err := config.Load(configFile); err != nil {
			return "", ExitCodeError(1, fmt.Errorf("loading config: %w", err))
		}
	}
	...
	runID, err := newUUIDV4()
	if err != nil {
		return "", ExitCodeError(2, fmt.Errorf("generating run ID: %w", err))
	}
	...
	if _, err := os.Stat(outputDir); err == nil {
		return "", ExitCodeError(2, fmt.Errorf("directory already exists: %s", outputDir))
	}

	// Step 6: create output directory and evidence/ subdirectory.
	if err := os.MkdirAll(outputDir, 0o750); err != nil {
		return "", ExitCodeError(2, fmt.Errorf("creating run directory: %w", err))
	}
	evidenceDir := filepath.Join(outputDir, "evidence")
	if err := os.MkdirAll(evidenceDir, 0o750); err != nil {
		return "", ExitCodeError(2, fmt.Errorf("creating evidence directory: %w", err))
	}
