...
func (o *Options) verifyPublisherSignature(ctx context.Context, dir string) (skipped bool, err error) {
...
// extractBinary pulls the single expected "pipelock" binary out of a release
...
func extractBinary(archive []byte, isZip bool, dir string) (string, error) {
	tmp, err := os.CreateTemp(dir, ".pipelock-update-*")
	if err != nil {
	...
	var writeErr error
	if isZip {
		writeErr = extractZip(archive, tmp)
	} else {
		writeErr = extractTarGz(archive, tmp)
	}
	if writeErr != nil {
		cleanup()
		return "", writeErr
...
// extractTarGz writes the entry whose base name equals binaryName into w.
func extractTarGz(archive []byte, w io.Writer) error {
	...
	for {
		...
		// Bounded copy: never write more than the download cap.
		if _, err := io.Copy(w, io.LimitReader(tr, maxDownloadBytes)); err != nil {
			return fmt.Errorf("extracting %q: %w", binaryName, err)
		}
		return nil
	}
	return fmt.Errorf("%w: %s not found in archive", ErrAssetNotFound, binaryName)
}

// extractZip writes the entry whose base name equals binaryName into w.
func extractZip(archive []byte, w io.Writer) error {
	...
	for _, f := range zr.File {
		...
		if f.FileInfo().IsDir() || base != binaryName {
			continue
		}
		rc, err := f.Open()
		if err != nil {
			return fmt.Errorf("opening %q in zip: %w", binaryName, err)
		}
		_, copyErr := io.Copy(w, io.LimitReader(rc, maxDownloadBytes)) //nolint:gosec // bounded by maxDownloadBytes
		_ = rc.Close()
		if copyErr != nil {
			return fmt.Errorf("extracting %q: %w", binaryName, copyErr)
		}
		return nil
	}
	return fmt.Errorf("%w: %s not found in archive", ErrAssetNotFound, binaryName)
...
func copyFile(src, dst string) error {
	...
	tmpPath := tmp.Name()
	if _, err := io.Copy(tmp, io.LimitReader(in, maxDownloadBytes)); err != nil {
		_ = tmp.Close()
		_ = os.Remove(tmpPath)
