...
func resetMintCmd() *cobra.Command {
	...
	cmd.Flags().StringVar(&instanceID, "instance", "", "live proxy instance ID from reset status output")
	cmd.Flags().Uint64Var(&epoch, "epoch", 0, "current reset epoch from reset status output")
