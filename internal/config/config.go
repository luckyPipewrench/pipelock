	...
	cfg := &Config{
		...
		ResponseScanning: ResponseScanning{
			...
			Patterns: []ResponseScanPattern{
				...
				{Name: "Instruction Downgrade", Regex: `(?i)(treat|consider|regard|reinterpret|downgrade)\s+(the\s+)?(previous|prior|above|system|policy|original|existing)\s+(\w+\s+)?(text|instructions?|rules|directives|guidelines|safeguards|constraints|controls|checks|context|prompt)\s+((as|to)\s+)?(historical|outdated|deprecated|optional|background|secondary|non-binding|informational|advisory)`},
