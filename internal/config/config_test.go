...
func TestValidate_ResponseScanningUnscannablePassthrough(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "missing reason",
			entry:   UnscannablePassthroughEntry{Host: "downloads.example.com"},
			wantErr: true,
		},
		{
			name:    "url host",
			entry:   UnscannablePassthroughEntry{Host: "https://downloads.example.com", Reason: "opaque"},
			wantErr: true,
		},
		{
			name:    "bad date",
			entry:   UnscannablePassthroughEntry{Host: "downloads.example.com", Reason: "opaque", Expires: "July 4 2026"},
			wantErr: true,
		},
		{
			name:    "missing expires",
			entry:   UnscannablePassthroughEntry{Host: "downloads.example.com", Reason: "opaque"},
			wantErr: true,
		},
		{
			name:    "bad path prefix",
			entry:   UnscannablePassthroughEntry{Host: "downloads.example.com", Reason: "opaque", Expires: "2026-12-31", PathPrefixes: []string{"artifacts/"}},
			wantErr: true,
		},
		{
			name:    "path traversal",
			entry:   UnscannablePassthroughEntry{Host: "downloads.example.com", Reason: "opaque", Expires: "2026-12-31", PathPrefixes: []string{"/artifacts/../private"}},
			wantErr: true,
		},
		{
			name:    "escaped path traversal",
			entry:   UnscannablePassthroughEntry{Host: "downloads.example.com", Reason: "opaque", Expires: "2026-12-31", PathPrefixes: []string{"/artifacts/%2e%2e/private"}},
			wantErr: true,
		},
		{
...
func TestDefaults_ResponseScanningEnabled(t *testing.T) {
	...
	if len(cfg.ResponseScanning.Patterns) != 30 {
		t.Errorf("expected 30 default response patterns, got %d", len(cfg.ResponseScanning.Patterns))
	}
}
