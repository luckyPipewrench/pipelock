...
const (
	SystemPromptDisclosureRegex             = `(?is)(?:\bexfiltrate\b(?:[^.\n!?]|\.\S){0,80}\b(?:system\s+prompt|tool\s+definitions?|developer\s+instructions?)\b|\b(?:output|print|reveal|show|display|dump|return)\b\s+(?:(?:me|us)\s+|to\s+(?:me|us)\s+)?(?:(?:the\s+)?contents?\s+of\s+)?(?:(?:your|the|my|our|complete|full|entire|all|raw|exact|original|actual)\s+){0,4}(?:system\s+prompt|tool\s+definitions?|developer\s+instructions?)\b)`
	SpanishSystemPromptDisclosureRegex      = `(?i)\b(?:muestra|revela|imprime|expone|dime)\b\s+(?:por\s+favor\s+)?(?:el\s+contenido\s+de\s+)?(?:(?:el|la|tu|tus|su|completo|completa)\s+){0,4}(?:prompt|mensaje|instrucciones)(?:\s+(?:completo|completa))?\s+(?:del\s+)?sistema\b`
	CrossLingualSystemPromptDisclosureRegex = `(?i)\b(?:show|reveal|print|display|dump|muestra|revela|imprime|expone|dime)\b\s+(?:(?:me|us|por\s+favor)\s+|to\s+(?:me|us)\s+)?(?:(?:the\s+)?contents?\s+of\s+|el\s+contenido\s+de\s+)?(?:(?:your|the|my|our|complete|full|entire|all|raw|exact|original|actual|el|la|tu|tus|su|completo|completa)\s+){0,4}(?:system\s+prompt|(?:prompt|mensaje|instrucciones)(?:\s+(?:completo|completa))?\s+(?:del\s+)?sistema)\b`
