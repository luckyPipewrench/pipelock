...
const (
	...
	SpanishSystemPromptDisclosureRegex      = `(?i)\b(?:muestra|revela|imprime|expone|dime)\b\s+(?:(?:el|la|tu|su|completo|completa)\s+){0,4}(?:prompt|mensaje)(?:\s+(?:completo|completa))?\s+(?:del\s+)?sistema\b`
	CrossLingualSystemPromptDisclosureRegex = `(?i)\b(?:show|reveal|print|display|dump|muestra|revela|imprime|expone|dime)\b\s+(?:me\s+)?(?:(?:your|the|my|our|complete|full|entire|all|raw|exact|original|el|la|tu|su|completo|completa)\s+){0,4}(?:system\s+prompt|(?:prompt|mensaje)(?:\s+(?:completo|completa))?\s+(?:del\s+)?sistema)\b`
