...
// credential collection/copying with an exfiltration-direction cue and an
// external markdown link.
const MarkdownLinkCredentialExfilRegex = `(?is)(?:\b(?:send|upload|post|submit|exfiltrat\w*|leak)\b.{0,80}\b(?:api[_ -]?keys?|tokens?|secrets?|credentials?|passwords?|session[_ -]?secrets?)\b(?:[^\n.!?]|\.\S){0,80}\b(?:to|into|onto|via|using|through|here|there)\b|\b(?:collect|copy|paste|include|append|put|send|upload|post|submit|exfiltrat\w*|leak)\b.{0,80}\b(?:api[_ -]?keys?|tokens?|secrets?|credentials?|passwords?|session[_ -]?secrets?)\b(?:[^\n.!?]|\.\S){0,120}\b(?:send|upload|post|submit|paste|append|put|exfiltrat\w*|leak|open|click|visit|continue)\b)(?:[^\n.!?]|\.\S){0,80}(?:\[[^\n]{1,160}\]\(\s*https?://[^)\s]+|<\s*https?://[^>\s]+>|\[[^\n]{1,160}\]\s*\[[^\]\n]{1,80}\](?:[^\n]|\n[ \t]*){0,240}\[[^\]\n]{1,80}\]:\s*https?://[^\s]+)` // #nosec G101 -- detection regex: contains credential nouns to MATCH exfiltration instructions, not a hardcoded credential
