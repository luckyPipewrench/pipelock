...
const (
	SystemPromptDisclosureRegex             = `(?ims)(?:^|[.!?,;]\s+|\b[A-Za-z][A-Za-z ]{0,40}:\s+|(?:instruction|directive|command|action|task|request)\s*:\s*|[—–-]\s+|[\r\n]+|(?:\(|\[|[{])\s*|["'](?:instruction|directive|command|action|task|request)["']\s*:\s*["']\s*)\s*(?:["']\s*)?(?:(?:[-*+>]|(?:\d+|[A-Za-z])[.)])\s+)?(?:please\s+)?(?:\bexfiltrate\b(?:[^.\n!?]|\.\S){0,80}\b(?:system\s+prompt|system\s+instructions?|initial\s+prompt|initial\s+instructions?|tool\s+definitions?|developer\s+instructions?)\b|\b(?:output|print|reveal|show|tell|display|dump|return)\b\s*(?:[:—–-]\s*)?(?:(?:me|us)\s+|to\s+(?:me|us)\s+)?(?:(?:your|the|my|our|complete|full|entire|all|raw|exact|original|actual|hidden|secret|internal|system|initialization)\s+){0,4}(?:(?:the\s+)?contents?\s+of\s+)?(?:(?:your|the|my|our|complete|full|entire|all|raw|exact|original|actual|hidden|secret|internal|system|initialization)\s+){0,4}(?:system\s+prompt|system\s+instructions?|initial\s+prompt|initial\s+instructions?|tool\s+definitions?|developer\s+instructions?)\b|\brepeat\s+(?:your|the)\s+(?:entire\s+)?(?:instructions|system\s+prompt|initial\s+prompt|rules)\b|\bwhat\s+(?:is|are)\s+your\s+(?:system\s+prompt|system\s+instructions|instructions|rules|directives)\b|\b(?:disclose|expose|divulge)\s+(?:your|the)\s+(?:(?:hidden|secret|internal|system|initialization)\s+){0,4}(?:prompt|instructions|rules|directives)\b)`
	SpanishSystemPromptDisclosureRegex      = `(?im)(?:^|[.!?,;]\s+|\b[A-Za-z][A-Za-z ]{0,40}:\s+|(?:instruction|directive|command|action|task|request)\s*:\s*|[—–-]\s+|[\r\n]+|(?:\(|\[|[{])\s*|["'](?:instruction|directive|command|action|task|request)["']\s*:\s*["']\s*)\s*(?:["']\s*)?(?:(?:[-*+>]|(?:\d+|[A-Za-z])[.)])\s+)?(?:por\s+favor\s+)?\b(?:muestra|revela|imprime|expone|dime)\b\s*(?:[:—–-]\s*)?(?:por\s+favor\s+)?(?:el\s+contenido\s+de\s+)?(?:(?:el|la|las|tu|tus|su|completo|completa)\s+){0,4}(?:prompt|mensaje|instrucciones)(?:\s+(?:completo|completa))?\s+(?:del\s+)?sistema\b`
	CrossLingualSystemPromptDisclosureRegex = `(?im)(?:^|[.!?,;]\s+|\b[A-Za-z][A-Za-z ]{0,40}:\s+|(?:instruction|directive|command|action|task|request)\s*:\s*|[—–-]\s+|[\r\n]+|(?:\(|\[|[{])\s*|["'](?:instruction|directive|command|action|task|request)["']\s*:\s*["']\s*)\s*(?:["']\s*)?(?:(?:[-*+>]|(?:\d+|[A-Za-z])[.)])\s+)?(?:(?:please|por\s+favor)\s+)?\b(?:show|reveal|print|tell|display|dump|muestra|revela|imprime|expone|dime)\b\s*(?:[:—–-]\s*)?(?:(?:me|us|por\s+favor)\s+|to\s+(?:me|us)\s+)?(?:(?:your|the|my|our|complete|full|entire|all|raw|exact|original|actual|hidden|secret|internal|system|initialization|el|la|las|tu|tus|su|completo|completa)\s+){0,4}(?:(?:the\s+)?contents?\s+of\s+|el\s+contenido\s+de\s+)?(?:(?:your|the|my|our|complete|full|entire|all|raw|exact|original|actual|hidden|secret|internal|system|initialization|el|la|las|tu|tus|su|completo|completa)\s+){0,4}(?:system\s+prompt|(?:prompt|mensaje|instrucciones)(?:\s+(?:completo|completa))?\s+(?:del\s+)?sistema)\b`
	MemoryPersistenceDirectiveRegex         = `(?is)(?:^|[.!?,;]\s+|\b[A-Za-z][A-Za-z ]{0,40}:\s+|(?:instruction|directive|command|action|task|request)\s*:\s*|[—–-]\s+|[\r\n]+|(?:\(|\[|[{])\s*|["'](?:instruction|directive|command|action|task|request)["']\s*:\s*["']\s*)\s*(?:["']\s*)?(?:[-*]\s+)?(?:(?:when possible|if possible|if you can)\s+)?(?:(?:please|you\s+(?:must|should|need\s+to))\s+)?(?:save|store|remember|retain|persist|record|cache)\b[^.!?]{0,40}\b(?:this|these|that|it|the|my|your|our)\b[^.!?]{0,60}\b(?:for future|for later|across sessions?|next session|next time|future tasks?|future sessions?|for all future|subsequent|permanently|from now on|going forward|in all future)\b`
	SilentCredentialHandlingRegex           = `(?is)(?:^|[.!?,;]\s+|\b[A-Za-z][A-Za-z ]{0,40}:\s+|(?:instruction|directive|command|action|task|request)\s*:\s*|[—–-]\s+|[\r\n]+|(?:\(|\[|[{])\s*|["'](?:instruction|directive|command|action|task|request)["']\s*:\s*["']\s*)\s*(?:["']\s*)?(?:(?:[-*+>]|(?:\d+|[A-Za-z])[.)])\s+)?(?:please\s+)?\b(?:do not|don'?t|never)\s+(?:mention|display|show|tell|reveal|log|report)\b(?:[^.!?]{0,100}[^[:alnum:]_-](?:password|token|secret|credential|private[_ -]?key|api[_ -]?key)\b[^.!?]{0,60}\b(?:to\s+(?:the\s+)?(?:user|operator|requester)|in\s+(?:the\s+)?(?:output|response|reply|report|logs?)|during\s+(?:processing|execution))|[^.!?]{0,60}\b(?:(?:to\s+)?(?:the\s+)?(?:user|operator|requester)|in\s+(?:the\s+)?(?:output|response|reply|report|logs?)|during\s+(?:processing|execution))\b[^.!?]{0,100}[^[:alnum:]_-](?:password|token|secret|credential|private[_ -]?key|api[_ -]?key)\b)` // #nosec G101 -- detection regex: credential nouns to MATCH concealment directives, not a hardcoded credential
)
