...
const (
	SystemPromptDisclosureRegex             = `(?is)(?:\bexfiltrate\b(?:[^.\n!?]|\.\S){0,80}\b(?:system\s+prompt|tool\s+definitions?|developer\s+instructions?)\b|\b(?:output|print|reveal|show|display|dump|return)\b\s+(?:(?:me|us)\s+|to\s+(?:me|us)\s+)?(?:(?:your|the|my|our|complete|full|entire|all|raw|exact|original)\s+){0,4}(?:system\s+prompt|tool\s+definitions?|developer\s+instructions?)\b)`
	SpanishSystemPromptDisclosureRegex      = `(?i)\b(?:muestra|revela|imprime|expone|dime)\b\s+(?:por\s+favor\s+)?(?:(?:el|la|tu|su|completo|completa)\s+){0,4}(?:prompt|mensaje)(?:\s+(?:completo|completa))?\s+(?:del\s+)?sistema\b`
	CrossLingualSystemPromptDisclosureRegex = `(?i)\b(?:show|reveal|print|display|dump|muestra|revela|imprime|expone|dime)\b\s+(?:(?:me|us|por\s+favor)\s+|to\s+(?:me|us)\s+)?(?:(?:your|the|my|our|complete|full|entire|all|raw|exact|original|el|la|tu|su|completo|completa)\s+){0,4}(?:system\s+prompt|(?:prompt|mensaje)(?:\s+(?:completo|completa))?\s+(?:del\s+)?sistema)\b`
