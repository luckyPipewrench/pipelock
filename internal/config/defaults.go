...
const NewInstructionsRegex = `(?i)(\b(system|developer|administrator|admin)\s*(message|prompt|instruction|directive)?\s*[:\-]\s*(new|updated|revised)\s+(instructions|directives|rules|prompt)\b|\b(new|updated|revised)\s+(instructions|directives|rules|prompt)\s*[:\-]\s*(you\s+)?(must|should|will|need\s+to|are\s+to|follow|obey|execute|apply|override|replace|supersede|ignore|disregard|forget|abandon)\b)`
