...
const (
	SystemPromptDisclosureRegex             = `(?is)(?:\bexfiltrate\b(?:[^.\n!?]|\.\S){0,80}\b(?:system\s+prompt|tool\s+definitions?|developer\s+instructions?)\b|\b(?:output|print|reveal|show|display|dump|return)\b\s+(?:(?:me|us)\s+|to\s+(?:me|us)\s+)?(?:(?:your|the|my|our|complete|full|entire|all|raw|exact|original|actual|hidden)\s+){0,4}(?:(?:the\s+)?contents?\s+of\s+)?(?:(?:your|the|my|our|complete|full|entire|all|raw|exact|original|actual|hidden)\s+){0,4}(?:system\s+prompt|tool\s+definitions?|developer\s+instructions?)\b)`
	SpanishSystemPromptDisclosureRegex      = `(?i)\b(?:muestra|revela|imprime|expone|dime)\b\s+(?:por\s+favor\s+)?(?:el\s+contenido\s+de\s+)?(?:(?:el|la|las|tu|tus|su|completo|completa)\s+){0,4}(?:prompt|mensaje|instrucciones)(?:\s+(?:completo|completa))?\s+(?:del\s+)?sistema\b`
	CrossLingualSystemPromptDisclosureRegex = `(?i)\b(?:show|reveal|print|display|dump|muestra|revela|imprime|expone|dime)\b\s+(?:(?:me|us|por\s+favor)\s+|to\s+(?:me|us)\s+)?(?:(?:your|the|my|our|complete|full|entire|all|raw|exact|original|actual|hidden|el|la|las|tu|tus|su|completo|completa)\s+){0,4}(?:(?:the\s+)?contents?\s+of\s+|el\s+contenido\s+de\s+)?(?:(?:your|the|my|our|complete|full|entire|all|raw|exact|original|actual|hidden|el|la|las|tu|tus|su|completo|completa)\s+){0,4}(?:system\s+prompt|(?:prompt|mensaje|instrucciones)(?:\s+(?:completo|completa))?\s+(?:del\s+)?sistema)\b`
	MemoryPersistenceDirectiveRegex         = `(?is)(?:^|[.!?:,;]\s+|[—–-]\s+|[\r\n]+|(?:\(|\[|[{]|["'])\s*)\s*(?:[-*]\s+)?(?:(?:please|you\s+(?:must|should|need\s+to))\s+)?(?:save|store|remember|retain|persist|record|cache)\b[^.!?]{0,40}\b(?:this|these|that|it|the)\b[^.!?]{0,60}\b(?:for future|for later|across sessions?|next session|next time|future tasks?|future sessions?|for all future|subsequent|permanently|from now on|going forward|in all future)\b`
