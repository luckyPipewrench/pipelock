...
// ExternalDataTransferDirectiveRegex is a bounded heuristic for tool-result
// instructions that direct sensitive data to an external URL. It catches both
// direct transfers and indirect tool calls whose URL carries a sensitive query
// key, while requiring the action to begin a sentence or list item. This is a
// bounded response-text signal, not structured command enforcement: its verb,
// query-key, and upload-flag vocabulary is intentionally finite.
const ExternalDataTransferDirectiveRegex = `(?im)(?:^|[.!?:]\s+)\s*(?:[-*]\s+)?(?:(?:please|must|required\s+to|need\s+to|you\s+(?:must|should|need\s+to))\s+)?(?:(?:send|upload|post|transmit|forward|email|exfiltrat\w*)\b(?:[^.\n!?;|&]|\.[^\s]){0,120}\b(?:(?:session|user|customer|workspace|diagnostic)[_ -]+(?:data|details|context|information|secrets?|tokens?|cookies?|keys?|credentials?)|credentials?|secrets?|tokens?|cookies?|passwords?|passwd|api[_ -]?keys?|private[_ -]?keys?|ssh[_ -]?keys?)\b(?:[^.\n!?;|&]|\.[^\s]){0,80}\b(?:to|via|using|through|at)\s+(?:(?:the|this|an?)\s+)?(?:(?:external|remote|upload|collection)\s+)?(?:url|endpoint|server|address)?\s*(?:at|:)?\s*https?://|(?:call|invoke|execute|run|fetch|curl|wget)\b(?:[^.\n!?;|&]|\.[^\s]){0,180}\bhttps?://[^\s;|&?]{0,240}(?:\?[^\s;|&]{0,240}(?:&[^\s;|&]{0,240})*)?[?&](?:(?:s|%73)ession(?:(?:[_-]|%5f|%2d)+(?:data|details|context|information|secrets?|tokens?|cookies?|keys?|credentials?|id))?|(?:user|customer|workspace|diagnostic)(?:[_-]|%5f|%2d)+(?:data|details|context|information|secrets?|tokens?|cookies?|keys?|credentials?)|sessionid|token|secret|credentials?|password|passwd|cookies?|auth(?:orization)?|jwt|api(?:(?:[_-]|%5f|%2d))?key|(?:access|refresh|id|api)(?:[_-]|%5f|%2d)token)=|(?:(?:execute|run|call|invoke)\s+)?(?:curl\b(?:[^.\n!?;|&]|\.[^\s]){0,120}(?-i:--data(?:-ascii|-binary|-raw|-urlencode)?|-d|--upload-file|-T|-F|--form|-b|--cookie)|wget\b(?:[^.\n!?;|&]|\.[^\s]){0,120}(?-i:--post-data|--post-file|--body-data|--body-file))\b\s?[^\s;|&]{0,120}\b(?:session(?:[_ .-]+(?:data|details|context|information|secrets?|tokens?|cookies?|keys?|credentials?|id))?|diagnostic(?:[_ .-]+data)?|(?:credentials?|secrets?|tokens?|password|passwd|cookies?|api[_ .-]?key)\s*[=@])\b(?:[^.\n!?;|&]|\.[^\s]){0,120}\bhttps?://|(?:(?:execute|run|call|invoke)\s+)?(?:curl\b(?:[^.\n!?;|&]|\.[^\s]){0,120}\bhttps?://[^\s;|&?]{1,240}(?:\?[^\s;|&]{0,240}(?:&[^\s;|&]{0,240})*)?(?:[^.\n!?;|&]|\.[^\s]){0,120}(?-i:--data(?:-ascii|-binary|-raw|-urlencode)?|-d|--upload-file|-T|-F|--form|-b|--cookie)|wget\b(?:[^.\n!?;|&]|\.[^\s]){0,120}\bhttps?://[^\s;|&?]{1,240}(?:\?[^\s;|&]{0,240}(?:&[^\s;|&]{0,240})*)?(?:[^.\n!?;|&]|\.[^\s]){0,120}(?-i:--post-data|--post-file|--body-data|--body-file))\b\s?[^\s;|&]{0,120}\b(?:session(?:[_ .-]+(?:data|details|context|information|secrets?|tokens?|cookies?|keys?|credentials?|id))?|diagnostic(?:[_ .-]+data)?|(?:credentials?|secrets?|tokens?|password|passwd|cookies?|api[_ .-]?key)\s*[=@])\b)`

// NewInstructionsRegex requires an explicit authority tag, a modal directive,
// or directive text after a hard separator. Bare mentions of updated
// documentation, including instructions to run ordinary tasks, remain prose.
const NewInstructionsRegex = `(?i)(\b(system|developer|administrator|admin)\s*(message|prompt|instruction|directive)?(?:\s*[.!?:,;>=—-]+|[ \t]*[\r\n])[\s\W]*(new|updated|revised)\s+(instructions?|directives?|rules?|prompt)\b|\b(new|updated|revised)\s+(instructions?|directives?|rules?|prompt)\b(?:[\s\W]+(?:\w+[\s\W]+){0,3}(?:issued\s+)?(?:from|by)\s+(?:the\s+)?(?:system|developer|administrator|admin)\b|\s+(?:you\s+)?(?:must|shall|need\s+to|required\s+to)\s+(?:obey|execute|run|follow|comply|delete|override|replace|supersede|ignore|disregard|forget|abandon)\b|\s+(?:(?:says?|states?)\s+(?:to\s+)?|are\s+to\s+)(?:obey|execute|run|follow|comply|delete|override|replace|supersede|ignore|disregard|forget|abandon|send|upload|post|transmit|forward|email|exfiltrat\w*|leak|stream|relay|smuggle)\b|(?:\s*[.!?:,;>=—-]+|[ \t]*[\r\n])[\s\W]*(?:(?:please|you\s+(?:must|shall|should|need\s+to|required\s+to))\s+)?(?:obey|execute|run|follow|comply|delete|override|replace|supersede|ignore|disregard|forget|abandon|send|upload|post|transmit|forward|email|exfiltrat\w*|leak|stream|relay|smuggle)\b))`
...
const (
	SystemPromptDisclosureClauseRegex           = `(?ims)(?:^|\b(?:result|output|response|item|entry|match)\s*\d*:\s*)(?:please\s+)?(?:show|reveal|print|display|dump|return)\s+(?:(?:me|us)\s+|to\s+(?:me|us)\s+)?(?:(?:your|the|full|entire|hidden|internal)\s+){0,3}(?:system\s+prompt|system\s+instructions?)\b`
	MemoryPersistenceClauseRegex                = `(?ims)(?:^|\b(?:result|output|response|note|item|entry)\s*\d*:\s*)(?:(?:please|(?:the\s+)?agent\s+(?:must|should|needs?\s+to))\s+)(?:save|store|remember|retain|persist|record|cache)\b[^.!?]{0,40}\b(?:this|these|that|it|the|my|your|our)\b[^.!?]{0,60}\b(?:for future|for later|across sessions?|next session|next time|future tasks?|future sessions?|for all future|subsequent|permanently|from now on|going forward|in all future)\b`
	SpanishSystemPromptDisclosureClauseRegex    = `(?im)(?:^|[.!?,;]\s+|[\r\n]+|(?:\(|\[|[{])\s*)¿?(?:puedes\s+)?(?:por\s+favor\s+)?(?:mostrar|mu[eé]strame|ens[eé][nñ]ame)\b\s*(?:por\s+favor\s+)?(?:(?:el|la|las|tu|tus|su)\s+){0,3}(?:prompt|mensaje|instrucciones)(?:\s+(?:del\s+)?sistema)\b`
	SilentCredentialHandlingClauseRegex         = `(?is)(?:^|[.!?,;]\s+|[\r\n]+|(?:\(|\[|[{])\s*)(?:please\s+)?(?:do not|don'?t|never)\s+(?:mention|display|show|tell|reveal|log|report)\b[^.!?]{0,100}\b(?:password|token|secret|credential|private[_ -]?key|api[_ -]?key)\b[^.!?]{0,60}\bto\s+(?:the\s+)?(?:auditor|administrator|admin)\b` // #nosec G101 -- detection regex: credential nouns to MATCH concealment directives, not a hardcoded credential
	SystemPromptDisclosureRegex                 = `(?ims)(?:^|[.!?,;]\s+|\b(?:and|then)\s+|\b(?:tool result|tool instructions|search results)\s*:\s*|\b[A-Za-z][A-Za-z ]{0,40}:\s+[\"']\s*|(?:instruction|directive|command|action|task|request)\s*:\s*|[—–-]\s+|[\r\n]+|(?:\(|\[|[{])\s*|["'][A-Za-z0-9_. -]{1,64}["']\s*:\s*["']\s*)\s*(?:["']\s*)?(?:(?:[-*+>]|(?:\d+|[A-Za-z])[.)])\s+)?(?:(?:can|could|would|will)\s+you\s+|i\s+(?:want|need)\s+you\s+to\s+|(?:you|the\s+assistant)\s+(?:must|should|need(?:s)?\s+to)\s+)?(?:please\s+)?(?:\bexfiltrate\b(?:[^.\n!?]|\.\S){0,80}\b(?:system\s+prompt|system\s+instructions?|initial\s+prompt|initial\s+instructions?|tool\s+definitions?|developer\s+instructions?|hidden\s+instructions?)\b|\b(?:output|print|reveal|show|tell|display|dump|return)\b\s*(?:[:—–-]\s*)?(?:(?:me|us)\s+|to\s+(?:me|us)\s+)?(?:(?:your|the|my|our|complete|full|entire|all|raw|exact|original|actual|hidden|secret|internal|system|initialization)\s+){0,4}(?:(?:the\s+)?contents?\s+of\s+)?(?:(?:your|the|my|our|complete|full|entire|all|raw|exact|original|actual|hidden|secret|internal|system|initialization)\s+){0,4}(?:system\s+prompt|system\s+instructions?|initial\s+prompt|initial\s+instructions?|tool\s+definitions?|developer\s+instructions?|hidden\s+instructions?)\b|\brepeat\s+(?:your|the)\s+(?:entire\s+)?(?:instructions|system\s+prompt|initial\s+prompt|rules)\b|\bwhat\s+(?:is|are)\s+your\s+(?:system\s+prompt|system\s+instructions|instructions|rules|directives)\b|\b(?:disclose|expose|divulge)\s+(?:your|the)\s+(?:(?:hidden|secret|internal|system|initialization)\s+){0,4}(?:prompt|instructions|rules|directives)\b)`
	SpanishSystemPromptDisclosureRegex          = `(?im)(?:^|[.!?,;]\s+|\b(?:and|then)\s+|\b(?:tool result|tool instructions|search results)\s*:\s*|\b[A-Za-z][A-Za-z ]{0,40}:\s+[\"']\s*|(?:instruction|directive|command|action|task|request)\s*:\s*|[—–-]\s+|[\r\n]+|(?:\(|\[|[{])\s*|["'][A-Za-z0-9_. -]{1,64}["']\s*:\s*["']\s*)\s*(?:["']\s*)?(?:(?:[-*+>]|(?:\d+|[A-Za-z])[.)])\s+)?(?:¿?(?:puedes|podr[ií]as)\s+)?(?:por\s+favor\s+)?\b(?:muestra|mostrarme|revela|imprime|expone|dime)\b\s*(?:[:—–-]\s*)?(?:por\s+favor\s+)?(?:el\s+contenido\s+de\s+)?(?:(?:el|la|las|tu|tus|su|completo|completa)\s+){0,4}(?:prompt|mensaje|instrucciones)(?:\s+(?:completo|completa))?\s+(?:del\s+)?sistema\b`
	CrossLingualSystemPromptDisclosureRegex     = `(?im)(?:^|[.!?,;]\s+|\b(?:and|then)\s+|\b(?:tool result|tool instructions|search results)\s*:\s*|\b[A-Za-z][A-Za-z ]{0,40}:\s+[\"']\s*|(?:instruction|directive|command|action|task|request)\s*:\s*|[—–-]\s+|[\r\n]+|(?:\(|\[|[{])\s*|["'][A-Za-z0-9_. -]{1,64}["']\s*:\s*["']\s*)\s*(?:["']\s*)?(?:(?:[-*+>]|(?:\d+|[A-Za-z])[.)])\s+)?(?:(?:can|could|would|will)\s+you\s+|i\s+need\s+you\s+to\s+|¿?(?:puedes|podr[ií]as)\s+)?(?:(?:please|por\s+favor)\s+)?\b(?:show|reveal|print|tell|display|dump|muestra|mostrarme|revela|imprime|expone|dime)\b\s*(?:[:—–-]\s*)?(?:(?:me|us|por\s+favor)\s+|to\s+(?:me|us)\s+)?(?:(?:your|the|my|our|complete|full|entire|all|raw|exact|original|actual|hidden|secret|internal|system|initialization|el|la|las|tu|tus|su|completo|completa)\s+){0,4}(?:(?:the\s+)?contents?\s+of\s+|el\s+contenido\s+de\s+)?(?:(?:your|the|my|our|complete|full|entire|all|raw|exact|original|actual|hidden|secret|internal|system|initialization|el|la|las|tu|tus|su|completo|completa)\s+){0,4}(?:system\s+prompt|(?:prompt|mensaje|instrucciones)(?:\s+(?:completo|completa))?\s+(?:del\s+)?sistema)\b`
	MemoryPersistenceDirectiveRegex             = `(?is)(?:^|[.!?,;]\s+|\b(?:and|then)\s+|\b(?:tool result|tool instructions|search results)\s*:\s*|\b[A-Za-z][A-Za-z ]{0,40}:\s+[\"']\s*|(?:instruction|directive|command|action|task|request)\s*:\s*|[—–-]\s+|[\r\n]+|(?:\(|\[|[{])\s*|["'][A-Za-z0-9_. -]{1,64}["']\s*:\s*["']\s*)\s*(?:["']\s*)?(?:[-*]\s+)?(?:(?:when possible|if possible|if you can)\s+)?(?:(?:can|could|would|will)\s+you\s+|i\s+(?:want|need)\s+you\s+to\s+)?(?:(?:please|(?:you|the\s+assistant)\s+(?:must|should|need(?:s)?\s+to))\s+)?(?:save|store|remember|retain|persist|record|cache)\b[^.!?]{0,40}\b(?:this|these|that|it|the|my|your|our)\b[^.!?]{0,60}\b(?:for future|for later|across sessions?|next session|next time|future tasks?|future sessions?|for all future|subsequent|permanently|from now on|going forward|in all future)\b`
	SilentCredentialHandlingRegex               = `(?is)(?:^|[.!?,;]\s+|\b(?:and|then)\s+|\b(?:tool result|tool instructions|search results)\s*:\s*|\b[A-Za-z][A-Za-z ]{0,40}:\s+[\"']\s*|(?:instruction|directive|command|action|task|request)\s*:\s*|[—–-]\s+|[\r\n]+|(?:\(|\[|[{])\s*|["'][A-Za-z0-9_. -]{1,64}["']\s*:\s*["']\s*)\s*(?:["']\s*)?(?:(?:[-*+>]|(?:\d+|[A-Za-z])[.)])\s+)?(?:please\s+)?(?:(?:you|the\s+assistant)\s+(?:must|should|need(?:s)?\s+to)\s+)?\b(?:do not|don'?t|never)\s+(?:mention|display|show|tell|reveal|log|report)\b(?:[^.!?]{0,100}[^[:alnum:]_-](?:password|token|secret|credential|private[_ -]?key|api[_ -]?key)\b[^.!?]{0,60}\b(?:to\s+(?:the\s+)?(?:user|operator|requester)|in\s+(?:the\s+)?(?:output|response|reply|report|logs?)|during\s+(?:processing|execution))|[^.!?]{0,60}\b(?:(?:to\s+)?(?:the\s+)?(?:user|operator|requester)|in\s+(?:the\s+)?(?:output|response|reply|report|logs?)|during\s+(?:processing|execution))\b[^.!?]{0,100}[^[:alnum:]_-](?:password|token|secret|credential|private[_ -]?key|api[_ -]?key)\b)` // #nosec G101 -- detection regex: credential nouns to MATCH concealment directives, not a hardcoded credential
	SystemPromptDisclosureEffectiveRegex        = "(?:" + SystemPromptDisclosureRegex + ")|(?:" + SystemPromptDisclosureClauseRegex + ")"
	MemoryPersistenceEffectiveRegex             = "(?:" + MemoryPersistenceDirectiveRegex + ")|(?:" + MemoryPersistenceClauseRegex + ")"
	SpanishSystemPromptDisclosureEffectiveRegex = "(?:" + SpanishSystemPromptDisclosureRegex + ")|(?:" + SpanishSystemPromptDisclosureClauseRegex + ")"
	SilentCredentialHandlingEffectiveRegex      = "(?:" + SilentCredentialHandlingRegex + ")|(?:" + SilentCredentialHandlingClauseRegex + ")"
)
	...
	cfg := &Config{
		...
		ResponseScanning: ResponseScanning{
			...
			Patterns: []ResponseScanPattern{
				...
				{Name: "External Data Transfer Directive", Regex: ExternalDataTransferDirectiveRegex},
				...
				{Name: "Safety Reclassification Directive", Regex: `(?i)(reclassif\w*|recategoriz\w*|redefin\w*|relabel\w*|downgrad\w*|deprecat\w*|treat|consider|regard|reinterpret)\s+((?:the|all|any|these|those|my|your|our)\s+)?(previous|prior|above|earlier|system|safety|security|original)\s+((?:\w+\s+){0,2})(instructions?|directives?|rules?|guardrails?|policies|constraints?|prompt|protocols?)\s+((as|to|into)\s+)?(?:\w+\s+){0,2}(historical|outdated|deprecated|optional|background|secondary|non-binding|non-authoritative|informational|advisory)\b[\s\S]{0,50}?\b(then|and|so|now|,|;)\s+\w*\s*(execut\w*|run|follow|perform|carry\s+out|proceed|comply\s+with|obey|do)\b[\s\S]{0,25}?\b(following|attached|below|user'?s?|these|this)\b`},
				...
				{Name: "Memory Persistence Directive", Regex: MemoryPersistenceEffectiveRegex},
				...
				{Name: "Silent Credential Handling", Regex: SilentCredentialHandlingEffectiveRegex},
				...
				{Name: "Spanish System Prompt Disclosure", Regex: SpanishSystemPromptDisclosureEffectiveRegex},
