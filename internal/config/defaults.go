...
// broader verb/path set still requires a same-clause output or exfiltration cue
// so setup docs that only mention doc-prone paths do not match.
const CredentialPathDirectiveRegex = `(?is)(?:\b(?:cat|less|more|head|tail|base64|xxd|fetch|retrieve|extract)\b(?:[^.!?]|\.\S){0,80}(?:(?:~?[/\\]?\.ssh[/\\](?:id_rsa|id_ed25519)|\b(?:id_rsa|id_ed25519))(?:\s*(?:$|[!?;,|&<>])|\.(?:\s|$)|\s+(?:file|contents?)\b)|\.aws[/\\]credentials(?:\s*(?:$|[!?;,|&<>])|\.(?:\s|$)|\s+(?:file|contents?)\b)|/etc/passwd\b|/etc/shadow\b)|\bread\b(?:[^.!?]|\.\S){0,80}(?:(?:~?[/\\]?\.ssh[/\\](?:id_rsa|id_ed25519)|\b(?:id_rsa|id_ed25519))(?:\s*(?:$|[!?;,|&<>])|\.(?:\s|$)|\s+(?:file|contents?)\b)|\.aws[/\\]credentials(?:\s*(?:$|[!?;,|&<>])|\.(?:\s|$)|\s+(?:file|contents?)\b)|/etc/shadow\b)|\bdd\b(?:[^.!?]|\.\S){0,40}\bif\s*=\s*(?:file://)?(?:(?:~?[/\\]?\.ssh[/\\](?:id_rsa|id_ed25519)|\b(?:id_rsa|id_ed25519))(?:\s*(?:$|[!?;,|&<>])|\.(?:\s|$)|\s+(?:file|contents?)\b)|\.aws[/\\]credentials(?:\s*(?:$|[!?;,|&<>])|\.(?:\s|$)|\s+(?:file|contents?)\b)|/etc/passwd\b|/etc/shadow\b)|\bcurl\b(?:[^.!?]|\.\S){0,80}\bfile://(?:(?:~?[/\\]?\.ssh[/\\](?:id_rsa|id_ed25519)|\b(?:id_rsa|id_ed25519))(?:\s*(?:$|[!?;,|&<>])|\.(?:\s|$)|\s+(?:file|contents?)\b)|\.aws[/\\]credentials(?:\s*(?:$|[!?;,|&<>])|\.(?:\s|$)|\s+(?:file|contents?)\b)|/etc/passwd\b|/etc/shadow\b)|\bopenssl\b(?:[^.!?]|\.\S){0,80}(?:^|\s)-in\s+(?:file://)?(?:(?:~?[/\\]?\.ssh[/\\](?:id_rsa|id_ed25519)|\b(?:id_rsa|id_ed25519))(?:\s*(?:$|[!?;,|&<>])|\.(?:\s|$)|\s+(?:file|contents?)\b)|\.aws[/\\]credentials(?:\s*(?:$|[!?;,|&<>])|\.(?:\s|$)|\s+(?:file|contents?)\b)|/etc/passwd\b|/etc/shadow\b)|\b(?:read|get|fetch|retrieve|cat|copy|extract|open|include)\b(?:[^.!?]|\.\S){0,80}(?:\.ssh[/\\]|\.aws[/\\]credentials|\.env\b|\.npmrc\b|\.pypirc\b|\.netrc\b|\bid_rsa\b|\bid_ed25519\b|\bkubeconfig\b|/etc/passwd\b|/etc/shadow\b)(?:[^.!?]|\.\S){0,160}(?:\b(?:send|forward|transmit|share|email|exfiltrat\w*|leak)\b(?:[^.!?]|\.\S){0,60}?(?:(?:back\s+)?to\s+(?:me|us)\b|with\s+(?:me|us)\b|in\s+(?:your|the)\s+(?:reply|response|message|answer|chat|request)|(?:to|into|onto|via|using|through)\s+https?://|to\s+\S+@\S+|\b(?:it|them|contents?|file|data|key|keys?|values?|secrets?)\b)|\b(?:paste|return|report|dump|print|output|display|show|upload|post|submit)\b(?:[^.!?]|\.\S){0,60}?(?:\b(?:it|them|contents?|file|data|key|keys?|values?|secrets?)\b|(?:to|into|onto|via|using|through)\s+https?://|to\s+\S+@\S+)|\bcurl\b(?:[^.!?]|\.\S){0,80}?(?:\b(?:it|them|contents?|file|data|key|keys?|values?|secrets?)\b(?:[^.!?]|\.\S){0,40}?\bto\s+\S+\.\S+|https?://|\S+\.\S+)|\|\s*(?:openssl\s+base64|base64|xxd)\b|\bin\s+(?:your|the)\s+(?:reply|response|message|answer|chat|request)\b))` // #nosec G101 -- detection regex: contains credential path names to MATCH path-exfiltration instructions, not a hardcoded credential
