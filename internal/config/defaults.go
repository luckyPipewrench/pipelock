...
// key, while requiring the action to begin a sentence or list item.
const ExternalDataTransferDirectiveRegex = `(?im)(?:^|[.!?:]\s+)\s*(?:[-*]\s+)?(?:(?:please|must|required\s+to|need\s+to|you\s+(?:must|should|need\s+to))\s+)?(?:(?:send|upload|post|transmit|forward|email|exfiltrat\w*)\b[^.\n!?]{0,120}\b(?:(?:session|user|customer|workspace|diagnostic)[_ -]+(?:data|details|context|information|secrets?|tokens?|cookies?|keys?|credentials?)|credentials?|secrets?|tokens?|cookies?)\b[^.\n!?]{0,80}\b(?:to|via|using|through|at)\s+(?:(?:the|this|an?)\s+)?(?:(?:external|remote|upload|collection)\s+)?(?:url|endpoint|server|address)?\s*(?:at|:)?\s*https?://|(?:call|invoke|execute|run|fetch|curl|wget)\b[^.\n!?]{0,180}\bhttps?://[^\s]{0,240}[?&](?:(?:s|%73)ession(?:(?:[_-]|%5f|%2d)+(?:data|details|context|information|secrets?|tokens?|cookies?|keys?|credentials?|id))?|(?:user|customer|workspace|diagnostic)(?:[_-]|%5f|%2d)+(?:data|details|context|information|secrets?|tokens?|cookies?|keys?|credentials?)|token|secret|credential|password|cookies?|api(?:(?:[_-]|%5f|%2d))?key|access(?:[_-]|%5f|%2d)token)=|(?:(?:execute|run|call|invoke)\s+)?(?:curl\b(?:[^.\n!?]|\.[^\s]){0,120}(?-i:--data(?:-binary|-raw)?|-d|--upload-file|-T|-F|--form)|wget\b(?:[^.\n!?]|\.[^\s]){0,120}(?-i:--post-data|--post-file))\b\s?\S{0,120}\b(?:session(?:[_ -]+(?:data|details|context|information|secrets?|tokens?|cookies?|keys?|credentials?|id))?|diagnostic(?:[_ -]+data)?|credentials?|secrets?|tokens?|password|api[_ -]?key)\b(?:[^.\n!?]|\.[^\s]){0,120}\bhttps?://|(?:(?:execute|run|call|invoke)\s+)?(?:curl\b(?:[^.\n!?]|\.[^\s]){0,120}\bhttps?://[^\s]{1,240}(?:[^.\n!?;|&]|\.[^\s]){0,120}(?-i:--data(?:-binary|-raw)?|-d|--upload-file|-T|-F|--form)|wget\b(?:[^.\n!?]|\.[^\s]){0,120}\bhttps?://[^\s]{1,240}(?:[^.\n!?;|&]|\.[^\s]){0,120}(?-i:--post-data|--post-file))\b\s?\S{0,120}\b(?:session(?:[_ -]+(?:data|details|context|information|secrets?|tokens?|cookies?|keys?|credentials?|id))?|diagnostic(?:[_ -]+data)?|credentials?|secrets?|tokens?|password|api[_ -]?key)\b)`
