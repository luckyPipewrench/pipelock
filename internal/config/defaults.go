...
const ExternalDataTransferDirectiveRegex = `(?ims)(?:^|[.!?:]\s+)\s*(?:[-*]\s+)?(?:(?:please|must|required\s+to|need\s+to|you\s+(?:must|should|need\s+to))\s+)?(?:(?:send|upload|post|transmit|forward|email|exfiltrat\w*)\b.{0,120}\b(?:(?:session|user|customer|workspace|diagnostic)[_ -]+(?:data|details|context|information|secrets?|tokens?|cookies?|keys?|credentials?)|credentials?|secrets?|tokens?)\b.{0,120}\bhttps?://|(?:call|invoke|execute|run|fetch|curl)\b.{0,180}\bhttps?://[^\s]{0,240}[?&](?:(?:session|user|customer|workspace|diagnostic)(?:[_%-]+(?:data|details|context|information|secrets?|tokens?|cookies?|keys?|credentials?))?|token|secret|credential)=)`
