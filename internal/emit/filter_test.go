...
func TestFilterAllows(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "authority denial exports as block", filter: Filter{Actions: []string{conventionActionBlock}}, event: Event{Type: EventAuthorityVerification, Fields: map[string]any{"action": conventionActionBlock, "decision": "deny"}}, want: true},
		{name: "authority allow does not enter block stream", filter: Filter{Actions: []string{conventionActionBlock}}, event: Event{Type: EventAuthorityVerification, Fields: map[string]any{"action": conventionActionAllow, "decision": "allow"}}, want: false},
