...
// Scorecard state constants. There is intentionally no aggregate
// "healthy/clean/OK" state: every line must be read on its own.
const (
	StateVerify  = "verify"
	StateWarn    = "warn"
	StateFail    = "fail"
...
// Chip label constants for scorecard lines.
const (
	chipAbsent           = "ABSENT"
...
// ComputeScorecard evaluates receipts against trusted keys and returns the
// scorecard and chain verification result.
...
// AbsentScorecard returns the scorecard for a session with no receipts.
...
// ReadLimitedScorecard returns a scorecard for a session where the dashboard
// read ceiling was reached before loading the complete session.
...
// SignerKeys returns the ordered, de-duplicated set of signer public keys
// (hex) across the given receipts.
...
// TrustedKeysForSession returns the subset of signers that appear in trustedKeys.
...
// FormatKeyList returns a comma-separated list of fingerprinted keys.
...
// Fingerprint returns the first 12 characters of a key for display.
