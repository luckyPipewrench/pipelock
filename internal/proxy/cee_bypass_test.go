...
func TestCEEBoundary_ExemptDomainIsEntropyOnly(t *testing.T) {
	...
	t.Run("entropy_budget_exempt", func(t *testing.T) {
		...
		res := ceeAdmit(context.Background(), ceeAdmitOptions{Outbound: []byte("x7k9mQ2pR4wL8nJ5vB3cT6yH0"), TargetURL: exemptURL, Agent: testCEEAgent, ClientIP: testCEEClientIP, RequestID: testCEERequestID, Config: ceeCfg, Entropy: et, Logger: logger, Metrics: m})
	...
	t.Run("fragment_dlp_still_fires", func(t *testing.T) {
		...
		r1 := ceeAdmit(context.Background(), ceeAdmitOptions{Outbound: []byte(testCEEAWSKeyPrefix), TargetURL: exemptURL, Agent: testCEEAgent, ClientIP: testCEEClientIP, RequestID: testCEERequestID, Config: ceeCfg, Entropy: et, Fragments: fb, Scanner: sc, Logger: logger, Metrics: m})
		if r1.Blocked {
			t.Fatal("first fragment should not block")
		}
		r2 := ceeAdmit(context.Background(), ceeAdmitOptions{Outbound: []byte(testCEEAWSKeySuffix), TargetURL: exemptURL, Agent: testCEEAgent, ClientIP: testCEEClientIP, RequestID: "req-2", Config: ceeCfg, Entropy: et, Fragments: fb, Scanner: sc, Logger: logger, Metrics: m})
