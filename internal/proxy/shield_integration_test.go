...
func TestProxy_ApplyShield_ExemptDomain(t *testing.T) {
	...
	result, summary, _, blocked := p.applyShield(body, "text/html", "hcaptcha.com", nil, cfg, actx, "127.0.0.1", "req1", TransportFetch, "act1")
	if blocked != nil {
...
func TestProxy_ApplyShield_NonShieldableContentBypassesOversize(t *testing.T) {
	...
	for _, tc := range nonShieldable {
		t.Run(tc.name, func(t *testing.T) {
			...
			result, summary, _, blocked := p.applyShield(body, tc.contentType, "example.com", nil, cfg, audit.LogContext{}, "127.0.0.1", "req1", TransportFetch, "act1")
			if blocked != nil {
...
func TestProxy_ApplyShield_SizeExemptUsesBoundedWholeBody(t *testing.T) {
	...
	result, summary, _, blocked := p.applyShield(body, "text/html", "docs.vendor.example", nil, cfg, audit.LogContext{}, "127.0.0.1", "req-size-exempt", TransportForward, "parent-action")
	if blocked != nil {
...
func TestProxy_ApplyShield_RecordsCappedAdaptiveSignals(t *testing.T) {
	...
	result, summary, _, blocked := p.applyShield(body, "text/html", "example.com", nil, cfg, actx, "127.0.0.1", "req-shield", TransportFetch, "parent-action")
	if blocked != nil {
