...
func EvidenceRunWriterGone(dir, session string) (bool, error) {
	return EvidenceWriterGone(filepath.Join(dir, RunWriterLockPrefix+session+RunWriterLockSuffix))
}

func acquireRunPresence(dir, session string) (*os.File, error) {
	path := filepath.Join(filepath.Clean(dir), RunWriterLockPrefix+session+RunWriterLockSuffix)
