...
func TestExtractLiteralPrefix(t *testing.T) {
	tests := []struct {
		...
		{"together", "(?i)tok_[a-z0-9]{40,}\\b", "tok_"},
		{"pinecone", "(?i)pcsk_[a-zA-Z0-9]{36,}", "pcsk_"},
		{"digitalocean", "(?i)dop_v1_[a-f0-9]{64}", "dop_v1_"},
		{"vault", `(?i)\bhvs\.[A-Za-z0-9]{24,}\b`, "hvs."},
		{"supabase", "(?i)\\bsb_secret_[A-Za-z0-9_-]{22}_(?:[A-Za-z0-9_-]{7}[A-Za-z0-9_]\\b|[A-Za-z0-9_-]{7}-\\B)", "sb_secret_"},
		{"npm", "(?i)npm_[A-Za-z0-9]{36,}\\b", "npm_"},
		{"pypi", "(?i)pypi-AgE[A-Za-z0-9_-]{90,}", "pypi-age"},
		{"linear", "(?i)\\blin_api_[A-Za-z0-9]{40,}\\b", "lin_api_"},
		{"notion", "(?i)ntn_[a-zA-Z0-9]{40,}\\b", "ntn_"},
		{"sentry", "(?i)\\bsntrys_[A-Za-z0-9]{40,}\\b", "sntrys_"},
		...
		{"vercel", "(?i)(?:vercel|vc[piark])_[a-zA-Z0-9]{24,}\\b", ""},
