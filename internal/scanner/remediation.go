...
const (
	bodyDLPOperatorKnob        = "Request body DLP matched. For a non-core false positive, add a top-level `suppress:` entry with `rule:` set to the matched rule name and `path:` scoped to the request path. Core DLP matches cannot be suppressed; fix the core pattern's precision. Built-in credential-audience patterns are also immutable: send the credential only to its declared audience; do not use an exemption or a global bypass."
	...
	headerDLPOperatorKnob               = "For a non-core false-positive header finding, add a top-level `suppress:` entry with `rule:` set to the matched DLP pattern and `path:` scoped to the exact request path. Core DLP matches cannot be suppressed; fix the core pattern's precision. Built-in credential-audience patterns are also immutable: send the credential only to its declared audience; do not use an exemption or a global bypass."
