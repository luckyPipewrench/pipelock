...
func TestStyleValueHidesCSSSyntax(t *testing.T) {
	tests := []struct {
		...
		{"crlf escape", "\\64\r\nisplay:none", true},
		{"crlf control", "dis\\\r\nplay:none", false},
		{"too long hides", `display:none;` + strings.Repeat("x", maxCSSStyleBytes), true},
		{"over the read limit hides", `color:red;` + strings.Repeat("x", maxCSSStyleBytes), true},
		{"hiding after long padding", `color:red;` + strings.Repeat("x", maxCSSStyleBytes-64) + `;display:none`, true},
		{"visible after long padding", `color:red;` + strings.Repeat("x", maxCSSStyleBytes-64), false},
		{"comment splits a keyword", "display:n/**/one", false},
		{"comment splits a hidden keyword", "visibility:hid/**/den", false},
